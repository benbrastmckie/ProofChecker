  ...
  return {
    ...
    tool: {
      ...
      analyze_context_reads: tool({
        description: "Show all context files that were read during the session (e.g., .opencode/agent/general.md)",
        args: {},
        async execute(args, context) {
          const { sessionID } = context
      ...
      check_context_compliance: tool({
        ...
        async execute(args, context) {
          ...
          lines.push(`According to General prompt, the agent should:`)
