...
- **research-001.md**: Identified 7 structural divergences between current BMCS completeness and standard semantics. Inventoried 7 Tier 1 Boneyard candidates (~6,191 lines). Confirmed BMCS completeness chain is sorry-free (modulo construction axiom).
...
- Move orphan files to Boneyard with documentation
...
- Proving the Finite Model Property (separate existing work)
...
|------|--------|------------|------------|
| Lean type issues with restricted WorldState subtype | M | M | Test early with a minimal compilation check; the subtype `{S : Set Formula // exists fam in B.families, S = fam.mcs 0}` may need careful universe handling |
...
| Constant-family temporal coherence requires careful proof | M | M | Reuse existing `temporal_backward_G`/`temporal_backward_H` from TemporalCoherentConstruction.lean which already handles this via contraposition |
| Convexity proof for canonical WorldHistory | L | L | Universal domain (fun _ => True) is trivially convex |
| Imports cause build issues due to new file | L | M | Build incrementally after each definition; keep imports minimal |

## Sorry Characterization

### Pre-existing Sorries
- `fully_saturated_bmcs_exists_int` in TemporalCoherentConstruction.lean (axiom, Int-specialized)
- ~12 sorries in TemporalCoherentConstruction.lean for temporal coherent construction
- 4 sorries in DovetailingChain.lean for cross-sign propagation
- 4 sorries in TruthLemma.lean for EvalBMCS structural limitations
...
### Pre-existing Axioms
- `fully_saturated_bmcs_exists_int` (Int-specialized, in TemporalCoherentConstruction.lean)
- `singleFamily_modal_backward_axiom` (FALSE, deprecated, in Construction.lean)

### Expected Resolution
- Phase 5 removes the FALSE axiom `singleFamily_modal_backward_axiom` from Construction.lean
- The `fully_saturated_bmcs_exists_int` axiom is NOT removed (still used by BMCS completeness chain)
...
- 1 pre-existing axiom for the BMCS construction (fully_saturated_bmcs_exists_int)
- The FALSE axiom is removed
- Zero-axiom target: both require constructive saturation proofs
...
### Phase 5: Standard Completeness Theorems and Cleanup [NOT STARTED]

- **Dependencies:** Phase 3, Phase 4
- **Goal:** Derive the three standard completeness theorems from the truth lemma, and clean up the codebase
...
- [ ] Prove `standard_strong_completeness : semantic_consequence Gamma phi -> ContextDerivable Gamma phi`
  ...
  - Contradiction
- [ ] Move Tier 1 Boneyard files (7 files, ~6,191 lines) to `Theories/Bimodal/Boneyard/Bundle/`:
  - PreCoherentBundle.lean, TemporalChain.lean, WeakCoherentBundle.lean, UnifiedChain.lean, ZornFamily.lean, TemporalLindenbaum.lean, FinalConstruction.lean
- [ ] Remove FALSE axiom `singleFamily_modal_backward_axiom` from Construction.lean (replace with comment noting deprecation)
- [ ] Update lakefile to exclude moved Boneyard files from build if needed
...
- `Theories/Bimodal/Metalogic/Bundle/Construction.lean` - remove FALSE axiom
- `Theories/Bimodal/Boneyard/Bundle/` (NEW directory) - moved orphan files
- `Theories/Bimodal/Metalogic/Bundle/PreCoherentBundle.lean` - move to Boneyard
- `Theories/Bimodal/Metalogic/Bundle/TemporalChain.lean` - move to Boneyard
- `Theories/Bimodal/Metalogic/Bundle/WeakCoherentBundle.lean` - move to Boneyard
- `Theories/Bimodal/Metalogic/Bundle/UnifiedChain.lean` - move to Boneyard
- `Theories/Bimodal/Metalogic/Bundle/ZornFamily.lean` - move to Boneyard
- `Theories/Bimodal/Metalogic/Bundle/TemporalLindenbaum.lean` - move to Boneyard
- `Theories/Bimodal/Metalogic/Bundle/FinalConstruction.lean` - move to Boneyard

**Verification:**
- lean_verify on standard_representation, standard_weak_completeness, standard_strong_completeness
- `lake build` succeeds with no errors
- Moved files no longer in Metalogic/Bundle/ directory
- FALSE axiom removed from Construction.lean
...
- [ ] Moved Boneyard files do not break any active imports
- [ ] FALSE axiom `singleFamily_modal_backward_axiom` is no longer referenced in active code
...
- `Theories/Bimodal/Boneyard/Bundle/` (NEW directory) - 7 moved orphan files
- `Theories/Bimodal/Metalogic/Bundle/Construction.lean` (MODIFIED) - FALSE axiom removed
- `specs/903_restructure_completeness_proof_bimodal_semantics/plans/implementation-001.md` (this file)
...
- Boneyard moves can be undone with `git checkout` on individual files
- FALSE axiom removal in Construction.lean is safe since it was already deprecated and not used in the active completeness chain
