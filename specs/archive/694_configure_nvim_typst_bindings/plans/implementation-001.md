...
Configure Neovim for Typst editing with a UX that mirrors the existing LaTeX setup. The modern 2026 approach uses tinymist LSP + treesitter (no typst.vim plugin required) + typst-preview.nvim for live preview. Keybindings will use `<leader>t` prefix (parallel to `<leader>l` for LaTeX), with nvim-surround bindings and snippets adapted for Typst syntax.
...
- **research-002.md**: CORRECTION - Modern Typst setup requires only tinymist + treesitter + typst-preview.nvim. typst.vim is NOT recommended for treesitter-enabled Neovim due to compatibility issues

## Goals & Non-Goals

**Goals**:
- Mirror LaTeX UX with `<leader>t` keybindings for Typst
...
- PDF viewer integration beyond Sioyek (already configured)
...
| Keybinding conflicts with existing mappings | Medium | Low | Use dedicated `<leader>t` prefix; verify no conflicts |
...
**Goal**: Create ftplugin with keybindings and nvim-surround configuration
...
- [ ] Add which-key bindings under `<leader>t`:
  - `<leader>tc` - compile
  - `<leader>te` - errors (diagnostics float)
  - `<leader>tf` - format
  - `<leader>tp` - preview toggle
  - `<leader>tP` - pin main file (multi-file projects)
  - `<leader>tv` - view PDF in Sioyek
...
**Verification**:
- Opening `.typ` file shows keybindings in which-key (`<leader>t`)
- Surround operations work (`ysiw*` for bold)
- `<leader>tp` toggles preview
...
- [ ] Test `<leader>tf` formatting with typstyle
- [ ] Test `<leader>tp` preview toggle
- [ ] Test `<leader>tP` pin main file in multi-file project
- [ ] Test `<leader>tc` compile command
- [ ] Test surround operations: add/change/delete bold, italic, math
- [ ] Test snippets: headings, math, figures, lists
- [ ] Verify no conflicts with LaTeX `<leader>l` bindings
...
- `~/.config/nvim/after/ftplugin/typst.lua` (new) - keybindings and surround
