...
**Tasks**:
- [ ] Rename in TemporalContent.lean (definition site)
- [ ] Update 15 files with usages (WitnessSeed.lean, CanonicalFrame.lean, ChainFMCS.lean, etc.)
- [ ] Run `lake build`
...
**Tasks**:
- [ ] Rename in WitnessSeed.lean (definition site)
- [ ] Update CanonicalFrame.lean, ConstructiveFragment.lean, WitnessSeedWrapper.lean
- [ ] Run `lake build`
...
### Phase 3: Migrate bmcs_* to BFMCS namespace [NOT STARTED]
...
**Tasks**:
- [ ] In BFMCS.lean: wrap in `namespace BFMCS ... end BFMCS`
...
**Verification**:
- `lake build` passes
- `grep -rn "\bbmcs_" Theories/Bimodal/Metalogic/` returns empty (code only)
- `B.reflexivity`, `B.transitivity`, `B.diamond_witness` resolve correctly

---

### Phase 4: Migrate set_mcs_* to SetMaximalConsistent namespace [NOT STARTED]
...
**Tasks**:
- [ ] Create `namespace SetMaximalConsistent` section in MCSProperties.lean
- [ ] Move 6 definitions from MCSProperties.lean into namespace
- [ ] Create namespace section in MaximalConsistent.lean, move 1 definition
- [ ] Create namespace section in Completeness.lean, move 9 definitions
- [ ] Update 19 files with call site changes (242 usages total)
- [ ] Run `lake build` after each source file modification
- [ ] Run full `lake build` at end
...
### Phase 5: Migrate mcs_* to SetMaximalConsistent namespace (selective) [NOT STARTED]
...
**Tasks**:
- [ ] For each file: verify first argument is `SetMaximalConsistent` type
- [ ] Create/extend namespace section in each file
- [ ] Move definitions into namespace
- [ ] Update call sites across 34 files
- [ ] Run `lake build` after each file batch
- [ ] Document any `mcs_*` NOT migrated (FMCS-related) with rationale
...
- [ ] Verify no remaining old names in code:
  - `GContent`, `HContent`
  - `ForwardTemporalWitnessSeed`, `PastTemporalWitnessSeed`
  - `bmcs_*` prefix
  - `set_mcs_*` prefix
  - Qualifying `mcs_*` prefix
- [ ] Verify dot notation works for all migrated namespaces:
  - `B.reflexivity`, `B.transitivity`, `B.diamond_witness`
  - `h_mcs.negation_complete`, `h_mcs.closed_under_derivation`, etc.
- [ ] Grep for `\bsorry\b` in modified files — confirm no sorries introduced
- [ ] Update README.md files with new naming conventions
- [ ] Document migration in module docstrings
...
- [ ] `grep -rn "\bsorry\b" <modified files>` unchanged from before
- [ ] `grep -n "^axiom " <modified files>` shows no new axioms

### General
- [ ] 192 GContent/HContent occurrences renamed
- [ ] 30 WitnessSeed occurrences renamed
- [ ] 3 bmcs_* theorems moved to BFMCS namespace
- [ ] 16 set_mcs_* theorems moved to SetMaximalConsistent namespace
- [ ] 14 mcs_* theorems moved to SetMaximalConsistent namespace
- [ ] Boneyard/ unchanged
