# Implementation Plan: Resolve is_valid_swap_involution Blocker
...
**Critical Discovery**: The theorem IS true for **derivable formulas only** (those provable in the proof system), where the temporal_duality rule guarantees swap preservation. The current usage at line 1172 (temporal_duality case) is within a derivation induction proof where this restriction applies.
...
4. **Document Resolution**: Clearly document why the original theorem was unprovable and the solution
...
- May need to use `rw` or `simp` to align types if Lean doesn't unify automatically

**Contingency**: Add a helper lemma that explicitly converts between the two forms if needed.
...
### Phase 2: Add Provable Theorem [NOT STARTED]

**Estimated Effort**: 2 hours

**Objective**: Implement the provable `derivable_valid_swap_involution` theorem restricted to derivable formulas.

**Tasks**:
1. Add new theorem after the axiom swap validity section (around line 1168)
2. Implement proof using temporal_duality rule and involution
3. Add comprehensive documentation explaining the restriction
4. Test compilation of the new theorem
...
- Documentation clearly explains the restriction
...
### Phase 3: Update temporal_duality Case [NOT STARTED]

**Estimated Effort**: 1.5 hours

**Objective**: Update the temporal_duality case in `derivable_implies_swap_valid` to use the new theorem.

**Tasks**:
1. Locate temporal_duality case (line 1226-1235)
2. Replace usage of `is_valid_swap_involution` with `derivable_valid_swap_involution`
3. Pass the derivation tree `h_ψ'` as the required argument
4. Adjust proof to handle any type alignment issues
5. Test compilation of the complete function
...
| temporal_duality ψ' h_ψ' ih =>
  ...
  rw [h_eq]
  -- h_ψ' : DerivationTree [] φ.swap_past_future (after rewrite)
...
- Comments accurately describe the proof steps
...
3. **Updated LEAN_STYLE_GUIDE.md**
   ...
   - Changes: Add section on semantic vs syntactic properties
...
2. **DerivationTree.temporal_duality Rule** (SATISFIED)
   ...
   - Located in `Logos/Core/ProofSystem/Derivation.lean`
   - Status: Complete and correct
...
- Phase 2: Add Provable Theorem - 2 hours
- Phase 3: Update temporal_duality Case - 1.5 hours
...
**Milestone 2: New Theorem Added** (2.5 hours cumulative)
- `derivable_valid_swap_involution` implemented and proven
- Theorem compiles without errors

**Milestone 3: temporal_duality Case Updated** (4 hours cumulative)
- temporal_duality case uses new theorem
- Complete function compiles

**Milestone 4: Build Verified** (5 hours cumulative)
