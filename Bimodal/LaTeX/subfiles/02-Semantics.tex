...
is a triple $\frame = (\worldstate, T, R)$ where:
\begin{itemize}
...
The nullity constraint ensures zero-duration tasks are identity. Compositionality ensures
...
\texttt{task\_rel} & \texttt{WorldState → T → WorldState → Prop} & Task relation \\
\texttt{nullity} & \texttt{∀ w, task\_rel w 0 w} & Identity constraint \\
...
A \textbf{world history} in task frame $\frame$ is a structure $\history$ with:
\begin{itemize}
...
Lean Field & Type & Description \\
\midrule
\texttt{domain} & \texttt{T → Prop} & Temporal domain \\
\texttt{convex} & \texttt{Convex domain} & Domain convexity \\
\texttt{states} & \texttt{(t : T) → domain t → WorldState} & State function \\
...
A \textbf{task model} over frame $\frame$ is a structure $\model$ with valuation:
\[
...
The temporal operators $\allpast$ and $\allfuture$ quantify over past and future times
within the history's domain.
...
\[
\forall T.\, \forall \frame : \text{TaskFrame}\, T.\, \forall \model.\,
