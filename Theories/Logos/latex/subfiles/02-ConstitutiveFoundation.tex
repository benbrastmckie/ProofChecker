...
\begin{itemize}
	\item $\tuple{P, \essence}$ is a complete lattice (the \emph{essence} order)
	\item $\tuple{P, \ground}$ is a complete lattice (the \emph{ground} order)
...
\begin{remark}\label{rem:bilattice-orders}
  The essence order $X \essence Y$ captures \emph{conjunctive containment}: (1) every verifier for $Y$ has a part that verifies $X$; (2) any fusion of a verifier for $X$ together with a verifier for $Y$ also verifies $Y$; and (3) every falsifier for $X$ also falsifies $Y$.
Intuitively, $X$ being the case is \textit{part of what it is for} $Y$ to be the case.
