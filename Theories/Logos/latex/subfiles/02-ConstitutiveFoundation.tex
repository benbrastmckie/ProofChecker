...
\begin{remark}[Negation Semantics]
...
\begin{remark}[Conjunction Semantics]
...
Crucially, $\fusion{t}{u}$ does \textit{not} verify ``Alice is thinking'' alone since $u$ is irrelevant to that claim---this non-monotonicity is characteristic of exact semantics.

The falsification clause is \textit{inclusive}: a conjunction is falsified by anything that falsifies either conjunct, or by a fusion of falsifiers for both.
This ensures that every verifier for $\metaA \land \metaB$ also verifies $\metaA \lor \metaB$, a natural uniformity condition.
\end{remark}
...
The verification clause is inclusive, mirroring the falsification clause for conjunction.
\end{remark}
...
A \emph{bilattice} is a structure $B = \tuple{P, \essence, \ground, \neg}$ where:
\begin{itemize}
	\item $\tuple{P, \essence}$ is a complete lattice (the \emph{essence} or ``information'' order)
	\item $\tuple{P, \ground}$ is a complete lattice (the \emph{ground} or ``truth'' order)
	\item $\neg : P \to P$ is an \emph{involutive negation}: $\neg\neg X = X$
	\item Negation exchanges the two orderings: $X \essence Y \iff \neg X \ground \neg Y$
\end{itemize}
A bilattice is \emph{interlaced} when its two orders interact uniformly:
$(X \star Z) \circ (Y \star Z)$ whenever $X \circ Y$, for any lattice operation $\star$ and either order $\circ$.
\end{definition}

\begin{remark}[Interpreting the Two Orders]\label{rem:bilattice-orders}
The essence order $X \essence Y$ captures \emph{conjunctive containment}:
every verifier for $Y$ has a part that verifies $X$, and every falsifier for $X$ also falsifies $Y$.
Intuitively, $X$ is ``essentially contained in'' $Y$---the positive content of $X$ is part of $Y$'s positive content.

The ground order $X \ground Y$ captures \emph{disjunctive containment}:
every falsifier for $Y$ has a part that falsifies $X$, and every verifier for $X$ also verifies $Y$.
Intuitively, $X$ ``grounds'' $Y$---the negative content of $X$ is part of $Y$'s negative content.
\end{remark}

\begin{remark}[Bilattice Structure of Bilateral Propositions]\label{rem:bilattice-structure}
The space of bilateral propositions forms a \emph{non-interlaced} bilattice:
\begin{itemize}
...
\end{itemize}
The failure of interlacing reflects the hyperintensional nature of the semantics:
giving up Boolean identities allows the two orders to diverge.
See Ginsberg (1988) and Fitting (1989--2002) for the general theory of bilattices.
\end{remark}
