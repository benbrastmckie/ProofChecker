...
% TODO: Revise the lean source code to match the definitions below.
...
\begin{definition}\label{def:constitutive-model}
    ...
    \item \textbf{n-place predicates} $\verifiertype{F}, \falsifiertype{F} : ((\Fin\;n \to \statespace) \to \statespace) \to \Prop$ are the \textit{verifier function} and \textit{falsifier function} types which are required to satisfy:
		      \begin{itemize}
...
% FIX: A definition should be provided here of the term algebra which matches that in the lean source code for the Logos/
