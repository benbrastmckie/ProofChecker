...
% FIX: include the universal quantifier
...
Object-language variables are denoted $v, v_1, v_2, v_3, \ldots$ to distinguish them from metalanguage variables $x, y, z$ which range over durations in the Dynamical Foundation.
When a single variable suffices (e.g., in lambda abstractions), we write simply $v$.
\end{remark}
...
The set of \emph{well-formed formulas} is defined inductively where $F$ over $n$-place predicates, $t_i$ over terms, and $v$ over variables:
  \[
    \metaA, \metaB \Define{}
      ...
      \top \mid \bot \mid
      \neg\metaA \mid
      (\metaA \land \metaB) \mid
      (\metaA \lor \metaB) \mid
      (\metaA \equiv \metaB) \mid
      (\lambda v.\metaA)(t)
  \]
\end{definition}

\noindent
See \leansrc{Logos.SubTheories.Foundation.Syntax}{ConstitutiveFormula}.
...
\begin{enumerate}
	\item \textbf{Material Conditional}: $\metaA \to \metaB \define \neg\metaA \lor \metaB$
	\item \textbf{Biconditional}: $\metaA \leftrightarrow \metaB \define (\metaA \to \metaB) \land (\metaB \to \metaA)$
\end{enumerate}
...
\begin{definition}\label{def:constitutive-model}
    ...
    \item \textbf{n-place predicates} $\verifiertype{F}, \falsifiertype{F} : ((\Fin\;n \to \statespace) \to \statespace) \to \Prop$ are the \textit{verifier function} and \textit{falsifier function} types which are required to satisfy:\footnote{An equivalent formulation takes $\verifiertype{F}, \falsifiertype{F} : \mathtt{Set}\;((\Fin\;n \to \statespace) \to \statespace)$ since in Lean 4, $\mathtt{Set}\;\alpha$ is defined as $\alpha \to \Prop$. Whereas the $\Set$ notation emphasizes the collective aspect, the $\Prop$ notation emphasizes the logical aspect; both capture the same mathematical structure.}
		      \begin{itemize}
...
\noindent
See \leansrc{Logos.Foundation.Model}{ConstitutiveModel}.
...
\begin{definition}\label{def:variable-assignment}
	A \emph{variable assignment} $\assignment : \mathrm{Var} \to \statespace$ maps each variable name to a state in the state space which represents an object.
\end{definition}
...
\[
	t \Define v \mid f(t_1, \ldots, t_n)
\]
where $v$ ranges over variables, $f$ over $n$-place function symbols, and $c$ is used to indicate constants 0-place function symbols.
...
\emph{Substitution} $t[s/v]$ (replacing $v$ with $s$ in $t$) is defined by:
\begin{align*}
	v'[s/v] &= \begin{cases} s & \text{if } v' = v \\ v' & \text{otherwise} \end{cases} \\
	% c[s/v] &= c \\
	f(t_1, \ldots, t_n)[s/v] &= f(t_1[s/v], \ldots, t_n[s/v])
\end{align*}
\end{definition}

% FIX: add a remark that it follows that c[s/v] = c

\noindent
See \leansrc{Logos.SubTheories.Foundation.Syntax}{Term}.

% FIX: define $v$-variant formally

\begin{definition}\label{def:variant-assignment}
	The \textit{$v$-variant} $\assignsub{s}{v}$ of the variable assignment $\assignment$ differs at most from $\assignment$ by assigning $v$ to $s$.
\end{definition}
...
\begin{definition}\label{def:term-extension}
	...
	\begin{itemize}
		\item \textbf{Variable} $\sem{v}^\assignment_\model = \assignment(v)$
...
\begin{definition}[Atomic Verification and Falsification]\label{def:atomic-verification}
	\begin{align}
    \model, \assignment, s \verifies F(t_1, \ldots, t_n)  & \iff \exists f \in \verifiertype{F} \text{ where } s = f(\sem{t_1}^\assignment_\model, \ldots, \sem{t_n}^\assignment_\model)  \\
    \model, \assignment, s \falsifies F(t_1, \ldots, t_n) & \iff \exists f \in \falsifiertype{F} \text{ where } s = f(\sem{t_1}^\assignment_\model, \ldots, \sem{t_n}^\assignment_\model)
	\end{align}
\end{definition}
...
\begin{definition}[Lambda Verification and Falsification]\label{def:lambda-verification}
	\begin{align}
		\model, \assignment, s \verifies (\lambda v.\metaA)(t)  & \iff \model, \assignsub{\sem{t}^\assignment_\model}{v}, s \verifies \metaA  \\
		\model, \assignment, s \falsifies (\lambda v.\metaA)(t) & \iff \model, \assignsub{\sem{t}^\assignment_\model}{v}, s \falsifies \metaA
	\end{align}
\end{definition}
...
\begin{definition}[Propositional Operations]\label{def:propositional-operations}
    ...
    \item \textbf{Essence}: $P \essence Q \define P \land Q = Q$
    \item \textbf{Ground}: $P \essence Q \define P \lor Q = Q$
	\end{itemize}
\end{definition}

% FIX: add a remark that these propositional operators are not to be confused with sentential operators, though it can be shown that a model induces a homomorphism from sentences to propositions on the same signature of operators, motivating the redundancy in notation
...
\subsection{Constitutive Consequence}\label{sec:constitutive-consequence}
...
\begin{definition}[Constitutive Consequence]\label{def:consequence-consequence}
