...
\begin{definition}\label{def:dynamical-frame}
	...
	\begin{itemize}
		...
		\item A ternary \textit{task relation} $\taskrel$ on $\statespace \times D \times \statespace$ satisfying the constraints below where $\task{s}{x}{t}$ indicates that there is a task from $s$ to $t$ in duration $x$:
    \begin{itemize}
