...
% FIX: the following don't look that good. Better to present the primitive symbol, and then in an indented item below provide an example with the intended reading for each.

% FIX: replace \mathsf{Stab} with \stability which should be defined to be \boxdot in the notation sty file, where the reading is: it is unpreventable that
...
\begin{itemize}
    ...
    \item \textbf{Temporal operators}: $\allpast$ (``it has always been that''), $\allfuture$ (``it is always going to be that'')
...
% FIX: spell out what constitutive foundation formulas are also included to make the definition explicit since otherwise this is not as general.
...
The set of \emph{well-formed formulas} extends the Constitutive Foundation:
  \[
    \metaA, \metaB \Define
      % FIX: add all the other constitutive foundations operators, though there are constraints on the arguments of \equiv to be a \metaA_\textsc{cf}
      \actual(t) \mid
      \nec\metaA \mid
      \allpast\metaA \mid
      \allfuture\metaA \mid
      \metaA \since \metaB \mid
      \metaA \until \metaB \mid
      \metaA_\textsc{cf} \boxright \metaB \mid
      \store{i}\metaA \mid
      \recall{i}\metaA
  \]
...
% FIX: integrate the definitions below into the above as well as into the Operator Summary below

\begin{definition}[Derived Operators]\label{def:derived-operators}
  The following notational conventions ease exposition where $\lambda v.\metaA$ binds the variable $v$ in $\metaA$ and $\forall$ is the universal quantifier applied to the resulting property, where $\exists v.\metaA$ is similar.
  \begin{enumerate}
    \item \textbf{Universal Quantification}: $\forall v.\metaA \define \forall(\lambda v.\metaA)$
    \item \textbf{Existential Quantification}: $\exists v.\metaA \define \neg \forall v. \neg\metaA$
  \end{enumerate}
...
See \leansrc{Logos.Dynamics.Syntax}{DynamicalFormula}.
...
% FIX: use the defined symbols for these properties and relations below, e.g., \compatible
...
% FIX: also define the exclusivity and exhaustivity constraints on bilateral propositions

\begin{definition}[Dynamical Model]\label{def:core-model}
  A \emph{dynamical model} is any $\model = \tuple{\statespace, \parthood, \temporalorder, \taskrel, \interp{\cdot}}$ where $\tuple{\statespace, \parthood, \temporalorder, \taskrel}$ is a dynamical frame and:
  \begin{itemize}
    \item $\interp{\cdot}$ is an interpretation defined in the \textit{Constitutive Foundation}
    \item % FIX: add exclusivity constraint
    \item % FIX: add exhaustivity constraint
  \end{itemize}
...
% FIX: explain that the modal structure makes it possible to extend the hyperintensional semantics from the Constitutive Foundations to assign verifiers and falsifiers for universally quantified sentences.

\subsubsection{Universal Quantification}\label{sec:universal-quantification}

\begin{definition}[Universal Quantification Verification and Falsification]\label{def:universal-quantification}
	\begin{align}
    \model, \assignment, s \verifies \forall \metaA(v)   \iff 
      & \text{(1) } \model, \assignsub{d}{v}, d \verifies \metaA \text{ for all } d \parthood s; \text{ and} \notag \\
      & \text{(2) } r \parthood s \text{ for all } r \compatible s \\
    \model, \assignment, s \falsifies \forall \metaA(v)   \iff 
      & \text{(1) } \model, \assignsub{d}{v}, d \falsifies \metaA \text{ for some } d \parthood s; \text{ and} \notag \\
      & \text{(2) } r \parthood s \text{ for all } r \compatible s 
	\end{align}
\end{definition}

% FIX: note that the other operators for tense, modality, etc., do not have hyperintensional semantic clauses (they are purely intensional operators)
...
% FIX: divide the following into two clauses for lambda, and two clauses for universal quantification, doing something similar for all the others below
...
\begin{align}
    ...
    \model, \tau, x, \assignment, \tempindex \trueat \forall\metaA(v) &\iff \model, \tau, x, \assignsub{s}{v}, \tempindex \trueat \metaA(v) \text{ for all } s : \statespace
\end{align}
  where $\metaA(v)$ is a property (a formula with at least one free variable $v$), and $\assignsub{s}{v}$ extends the assignment $\assignment$ by mapping $v$ to $s$.
\end{definition}
