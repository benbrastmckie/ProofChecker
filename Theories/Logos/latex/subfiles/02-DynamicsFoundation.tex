...
The \textit{Dynamical Foundation} extends the \textit{Constitutive Foundation} with temporal structure and a task relation, enabling evaluation of truth relative to world-histories and times.
While the hyperintensional foundation remains (distinguishing propositions by their exact verifiers and falsifiers), this extension adds intensional evaluation relative to contextual parameters (world-history, time, variable assignment) to determine truth-values for all \textit{Dynamical Foundation} sentences.
...
\begin{enumerate}
  \item All \textit{Constitutive Foundation} sentences ($F$, $f$, $x$, $\neg$, $\land$, $\lor$, $\top$, $\bot$, $\equiv$, $\lambda$, $=$)
	...
	\item Counterfactual sentences: $\metaA \boxright \metaB$, $\metaA \diamondright \metaB$
	\item Indexed sentences: $\store{i}\metaA$, $\recall{i}\metaA$
	\item Universal quantification: $\forall\metaA$, $\exists\metaA$
...
\begin{definition}[Core Frame]\label{def:core-frame}
	A \emph{core frame} is a structure $\mframe = \tuple{\statespace, \parthood, \temporalorder, \taskrel}$ where:
	\begin{itemize}
...
\begin{definition}[State Modality]\label{def:state-modality}
	Let $\mframe = \tuple{\statespace, \parthood, \temporalorder, \taskrel}$ be a core frame.
	\begin{align}
...
See \leansrc{Logos.Dynamics.Frame}{CoreFrame} for the Lean implementation.
...
\begin{definition}[Core Model]\label{def:core-model}
	...
	\begin{itemize}
		\item $\tuple{\statespace, \parthood, \temporalorder, \taskrel}$ is a core frame
		\item $\interp$ is an interpretation as in the Constitutive Foundation
	\end{itemize}
\end{definition}

See \leansrc{Logos.Dynamics.Model}{CoreModel} for the Lean implementation.
...
\begin{definition}[Core Tense Operators]\label{def:tense-truth}
	\begin{align}
		\model, \history, x, \assignment, \tempindex \trueat \always\metaA     & \iff \model, \history, y, \assignment, \tempindex \trueat \metaA \text{ for all } y \in D \text{ where } y < x  \\
		\model, \history, x, \assignment, \tempindex \trueat \willalways\metaA & \iff \model, \history, y, \assignment, \tempindex \trueat \metaA \text{ for all } y \in D \text{ where } y > x  \\
		\model, \history, x, \assignment, \tempindex \trueat \waspast\metaA    & \iff \model, \history, y, \assignment, \tempindex \trueat \metaA \text{ for some } y \in D \text{ where } y < x \\
		\model, \history, x, \assignment, \tempindex \trueat \willfuture\metaA & \iff \model, \history, y, \assignment, \tempindex \trueat \metaA \text{ for some } y \in D \text{ where } y > x
	\end{align}
\end{definition}
