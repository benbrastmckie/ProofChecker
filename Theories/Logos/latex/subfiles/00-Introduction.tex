...
\node[widebox] (foundation) {
  \begin{tabular}{c}
    ...
    {\small identity ($\equiv$), grounding ($\leq$), essence ($\sqsubseteq$)}
  \end{tabular}
};
...
\node[widebox, below=1.2cm of foundation] (explanatory) {
  \begin{tabular}{c}
    ...
    {\small modal ($\Box$, $\Diamond$) \quad temporal ($H$, $G$, $P$, $F$) \quad counterfactual ($\boxright$, $\diamondright$) \quad causal ($\circleright$, $\dotcircleright$)}
  \end{tabular}
};
...
\node[draw=none, left=0.4cm of epistemic, font=\Large] (leftellipsis) {$\cdots$};
...
\begin{pgfonlayer}{background}
  \node[fill=gray!15, draw=gray!40, rounded corners=6pt,
        fit=(leftellipsis)(epistemic)(normative)(spatial)(rightellipsis)(topspacer),
        inner sep=12pt,
        label={[font=\footnotesize\itshape, anchor=north, yshift=-8pt]north:Middle extensions are modular---combine any subset}] (middlebox) {};
\end{pgfonlayer}

% Bottom Layer: Agential and Reflection (side by side, same level)
% Position them symmetrically for straight vertical arrows
\node[box, below=2.8cm of normative, xshift=-2.8cm] (agential) {
  \begin{tabular}{c}
...
\node[box, below=2.8cm of normative, xshift=2.8cm] (reflection) {
  \begin{tabular}{c}
