...
\node[widebox, below=0.85cm of foundation] (explanatory) {
  \begin{tabular}{c}
...
\node[smallbox, below=2.0cm of middlebox] (social) {
  \begin{tabular}{c}
    \textbf{Social} \\
    {\footnotesize common knowledge,} \\
    {\footnotesize communication} \\
    {\small ($\mathsf{C}$, $\mathsf{Ann}$)}
  \end{tabular}
};
...
\node[smallbox, left=0.3cm of social] (agential) {
  \begin{tabular}{c}
    \textbf{Agential} \\
    {\footnotesize agent-indexed} \\
    {\footnotesize operators} \\
    {\small ($\mathsf{B}_a$, $\mathsf{K}_a$)}
  \end{tabular}
};
