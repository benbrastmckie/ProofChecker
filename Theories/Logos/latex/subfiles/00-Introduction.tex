...
\node[widebox, minimum height=1.8cm, below=0.85cm of foundation] (explanatory) {
  \begin{tabular}{c}
    ...
    {\small modal ($\Box$, $\Diamond$),~ temporal ($H$, $G$, $P$, $F$, $S$, $U$),~ counterfactual ($\boxright$, $\diamondright$),~ causal ($\circleright$, $\dotcircleright$)} \\
    {\small store/recall ($\downarrow$, $\uparrow$),~ quantifiers ($\forall$, $\exists$),~ actuality ($@$)}
  \end{tabular}
};
