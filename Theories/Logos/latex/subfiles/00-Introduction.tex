...
\node[widebox] (foundation) {
  \begin{tabular}{c}
    ...
    {\small $\equiv$ (identity), $\leq$ (grounding), $\sqsubseteq$ (essence)}
  \end{tabular}
};
...
\node[widebox, below=1.2cm of foundation] (explanatory) {
  \begin{tabular}{c}
    ...
    {\small $\Box$, $\Diamond$ (modal) \quad $\mathbf{H}$, $\mathbf{G}$, $\mathbf{P}$, $\mathbf{F}$ (temporal) \quad $\boxright$, $\Diamond\!\to$ (counterfactual)}
  \end{tabular}
};
...
\node[box, below=2.8cm of epistemic, xshift=0.5cm] (agential) {
  \begin{tabular}{c}
    \textbf{Agential Extension} \\
    {\small $\mathsf{B}_a$, $\mathsf{K}_a$ (agent-indexed)}
  \end{tabular}
};

\node[box, below=2.8cm of spatial, xshift=-0.5cm] (reflection) {
  \begin{tabular}{c}
    \textbf{Reflection Extension} \\
    {\small $\mathsf{I}$ (first-person metacognition)}
  \end{tabular}
};
...
% Middle box -> Agential and Reflection (two arrows from middle box)
\draw[arrow] (middlebox.south) ++(-2cm,0) -- ++(0,-0.8cm) -- (agential.north);
\draw[arrow] (middlebox.south) ++(2cm,0) -- ++(0,-0.8cm) -- (reflection.north);
