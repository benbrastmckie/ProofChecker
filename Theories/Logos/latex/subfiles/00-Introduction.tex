...
\begin{enumerate}
  \item \textbf{Constitutive Foundation}:
        Provides the hyperintensional base with predicates ($F$, $G$), functions ($f$, $g$), variables ($x$, $y$), booleans ($\neg$, $\land$, $\lor$), extremal ($\bot$, $\top$), lambda abstraction ($\lambda$), identity ($=$), and constitutive operators (propositional identity $\equiv$, grounding $\leq$, essence $\sqsubseteq$, reduction $\Rightarrow$) over a state lattice with bilateral propositions (verifier/falsifier pairs).
