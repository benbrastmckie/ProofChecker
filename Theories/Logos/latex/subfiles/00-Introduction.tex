...
The Logos is interpreted over a hyperintensional \textit{state semantics} which comprises the \textit{Constitutive Foundation} of the theory.
...
These contents can then be used to interpret standard first-order resources along with constitutive explanatory operators for \textit{essence}, \textit{ground}, \textit{reduction}, and \textit{propositional identity}.

The \textit{Dynamical Foundation} extends the state semantics to include a \textit{task relation} parameterized by durations belonging to a \textit{temporal order} that govern the lawful transitions by which a system and its parts evolve.
