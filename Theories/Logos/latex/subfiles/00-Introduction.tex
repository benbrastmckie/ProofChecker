...
The \textit{Dynamical Foundation} extends state frames to include a \textit{task relation} parameterized by the durations belonging to a \textit{temporal order} that govern the lawful transitions by which the states in a system are able to evolve.
These elements are then used to construct \textit{world sates} which represent total instantaneous states of a system, as well as \textit{world histories} which model the possible evolutions of that system by mapping durations to world states.
