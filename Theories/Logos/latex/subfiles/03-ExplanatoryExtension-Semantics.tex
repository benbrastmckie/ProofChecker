...
\begin{definition}[Atomic Truth]\label{def:atomic-truth}
	\begin{align}
		\model, \history, x, \assignment, \tempindex \satisfies F(t_1, \ldots, t_n)    & \iff \exists i : \mathtt{Fin}\;(\verifiercount{F}) : \verifierfn{F}{i}(\lambda j : \mathtt{Fin}\;n, \sem{t_j}^\assignment_\model) \parthood \history(x)   \\
		\model, \history, x, \assignment, \tempindex \notsatisfies F(t_1, \ldots, t_n) & \iff \exists i : \mathtt{Fin}\;(\falsifiercount{F}) : \falsifierfn{F}{i}(\lambda j : \mathtt{Fin}\;n, \sem{t_j}^\assignment_\model) \parthood \history(x)
	\end{align}
\end{definition}
...
\begin{remark}
	...
	This justifies using $\satisfies$ alone for truth and $\notsatisfies$ for falsehood.
\end{remark}
...
\begin{definition}[Counterfactual Conditional]\label{def:counterfactual-truth}
	\[
		\model, \history, x, \assignment, \tempindex \satisfies \metaphi \boxright \metaC \iff \text{for all } i : \mathtt{Fin}\;(\verifiercount{\metaphi}), t := \verifierfn{\metaphi}{i} \text{ and } \beta \in \historyspace:
	\]
