...
\begin{definition}[Atomic Truth]\label{def:atomic-truth}
	\begin{align}
		\model, \history, x, \assignment, \tempindex \satisfies F(t_1, \ldots, t_n)    & \iff \text{there is some } f \in \verifierset{F} \text{ where } f(\lambda i : \mathtt{Fin}\;n, \sem{t_i}^\assignment_\model) \parthood \history(x)  \\
		\model, \history, x, \assignment, \tempindex \notsatisfies F(t_1, \ldots, t_n) & \iff \text{there is some } f \in \falsifierset{F} \text{ where } f(\lambda i : \mathtt{Fin}\;n, \sem{t_i}^\assignment_\model) \parthood \history(x)
	\end{align}
\end{definition}
