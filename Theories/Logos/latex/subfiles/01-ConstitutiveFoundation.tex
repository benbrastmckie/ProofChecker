...
\begin{definition}\label{def:constitutive-model}
	...
	\begin{itemize}
		...
		\item \textbf{Constant symbols} $\interp{c} : (\mathtt{Fin}\;0 \to \statespace) \to \statespace$
		\item \textbf{n-place predicates} $\interp{F} = \langle \verifiertype{F}, \falsifiertype{F} \rangle$ where:
		      \begin{itemize}
			      \item $\verifiertype{F} = \{f : (\mathtt{Fin}\;n \to \statespace) \to \statespace \mid \forall\,\text{args} : \mathtt{Fin}\;n \to \statespace, \forall\,i < n, \text{args}\;i \parthood f(\text{args})\}$ is the \textit{verifier function type}
			      \item $\falsifiertype{F} = \{f : (\mathtt{Fin}\;n \to \statespace) \to \statespace \mid \forall\,\text{args} : \mathtt{Fin}\;n \to \statespace, \forall\,i < n, \text{args}\;i \parthood f(\text{args})\}$ is the \textit{falsifier function type}
		      \end{itemize}
		\item \textbf{Sentence letters} $\interp{p} = \langle \verifiertype{p}, \falsifiertype{p} \rangle$ of verifier and falsifier state sets
	\end{itemize}
\end{definition}
