...
\begin{definition}\label{def:constitutive-model}
	...
	\begin{itemize}
		...
		\item \textbf{n-place predicates} $\interp{F} = \langle \verifierset{F}, \falsifierset{F} \rangle$ where:
		      \begin{itemize}
			      \item $\verifierset{F} : \text{Set}((\mathtt{Fin}\;n \to \statespace) \to \statespace)$ is the set of \textit{verifier functions} for which $\Fusion(a_1, \ldots, a_n) \sqsubseteq f^+(a_1, \ldots, a_n)$ for any $f^+ \in \verifierset{F}$ and $a_1, \ldots, a_n : \statespace$.
			      \item $\falsifierset{F} : \text{Set}((\mathtt{Fin}\;n \to \statespace) \to \statespace)$ is the set of \textit{falsifier functions} for which $\Fusion(a_1, \ldots, a_n) \sqsubseteq f^-(a_1, \ldots, a_n)$ for any $f^- \in \falsifierset{F}$ and $a_1, \ldots, a_n : \statespace$.
		      \end{itemize}
		\item \textbf{Sentence letters} $\interp{p} = \langle \verifierset{p}, \falsifierset{p} \rangle$ where:
		      \begin{itemize}
			      \item $\verifierset{p} : \text{Set}(\statespace)$ is the set of \textit{verified states}.
			      \item $\falsifierset{p} : \text{Set}(\statespace)$ is the set of \textit{falsified states}.
		      \end{itemize}
	\end{itemize}
\end{definition}
