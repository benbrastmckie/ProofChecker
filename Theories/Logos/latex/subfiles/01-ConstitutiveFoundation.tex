...
\begin{definition}\label{def:constitutive-model}
    ...
    \item \textbf{n-place predicates} $\verifiertype{F}, \falsifiertype{F} : (\Fin\;n \to \statespace) \to \statespace \to \Prop$ are the \textit{verifier function type} and \textit{falsifier function type} which are required to satisfy:
		      \begin{itemize}
...
\begin{remark}\label{rem:trivial-cases}
	...
	\begin{itemize}
		...
		\item \textbf{Sentence letters} $\verifiertype{p}, \falsifierset{p} : \statespace \to \Prop$ are the \textit{verifier function type} and \textit{falsifier function type} for 0-place predicates.
	\end{itemize}
\end{remark}
