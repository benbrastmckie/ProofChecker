...
\begin{itemize}
  \item \textbf{n-place function symbols} $\interp{f} : \statespace^n \to \statespace$ where $\interp{c} : \statespace$ for constants 
...
\begin{align}
\model, \assignment, s \verifies F(t_1, \ldots, t_n) &\iff \exists f^+ : \verifierset{F} \text{ where } s = f^+(\sem{t_1}^\assignment_\model, \ldots, \sem{t_n}^\assignment_\model) \\
  \model, \assignment, s \falsifies F(t_1, \ldots, t_n) &\iff \exists f^- : \falsifierset{F} \text{ where } s = f^-(\sem{t_1}^\assignment_\model, \ldots, \sem{t_n}^\assignment_\model)
\end{align}
\end{definition}
...
\begin{remark}
Assignment $\assignsub{v}{x}$ differs at most from $\assignment$ by assigning $x$ to $v$.
\end{remark}
