...
% TODO: Use $\verifiertype{F} : (\Fin\;n \to \statespace) \to \statespace \to \Prop$ instead. It is important that \falsifiertype{F} be defined similarly, where \verifiertype{F} and \falsifiertype{F} may then have different extensions (i.e., where different verifier and falsifier functions inhabit them). I want to maintain consistent dependent type theory definitions wherever possible throughout, maintaining consistency between the latex and lean source code.
...
\begin{definition}\label{def:constitutive-model}
	...
	\begin{itemize}
		...
		\item \textbf{n-place predicates} $\interp{F} = \tuple{\verifierset{F}, \falsifierset{F}}$ where:
		      \begin{itemize}
			      \item $\verifierset{F} : \Set((\Fin\;n \to \statespace) \to \statespace)$ is set of \textit{verifier functions} for which $\Fusion(a_1, \ldots, a_n) \sqsubseteq f^+(a_1, \ldots, a_n)$ for any $f^+ \in \verifierset{F}$ and $a_1, \ldots, a_n : \statespace$ and $\fusion{f^+}{g^+} \in \verifierset{F}$ whenever both $f^+, g^+ \in \verifierset{F}$.
			      \item $\falsifierset{F} : \Set((\Fin\;n \to \statespace) \to \statespace)$ is set of \textit{falsifier functions} for which $\Fusion(a_1, \ldots, a_n) \sqsubseteq f^-(a_1, \ldots, a_n)$ for any $f^- \in \falsifierset{F}$ and $a_1, \ldots, a_n : \statespace$ and $\fusion{f^-}{g^-} \in \falsifierset{F}$ whenever both $f^-, g^- \in \falsifierset{F}$.
		      \end{itemize}
	\end{itemize}
\end{definition}

% TODO: these should be `\verifiertype{p} : \statespace \to \Prop` instead of `\Set(\statespace)` and similarly for \falsifiertype{p}. This separate notation shouldn't be required in the Lean source code since 'p' is the same as a 0-plance predicate 'F'.
...
\begin{remark}\label{rem:trivial-cases}
	...
	\begin{itemize}
		\item \textbf{Constant symbols} $\interp{c} : \statespace$.
		\item \textbf{Sentence letters} $\interp{p} = \tuple{\verifierset{p}, \falsifierset{p}}$ where:
		      \begin{itemize}
			      \item $\verifierset{p} : \Set(\statespace)$ is the set of \textit{verifier states}.
			      \item $\falsifierset{p} : \Set(\statespace)$ is the set of \textit{falsifier states}.
		      \end{itemize}
	\end{itemize}
\end{remark}
...
\begin{definition}[Atomic Verification and Falsification]\label{def:atomic-verification}
	\begin{align}
		\model, \assignment, s \verifies F(t_1, \ldots, t_n)  & \iff \exists f \in \verifierset{F} : s = f(\lambda i : \Fin\;n, \sem{t_i}^\assignment_\model)  \\
		\model, \assignment, s \falsifies F(t_1, \ldots, t_n) & \iff \exists f \in \falsifierset{F} : s = f(\lambda i : \Fin\;n, \sem{t_i}^\assignment_\model)
	\end{align}
\end{definition}
...
\begin{definition}[Bilateral Proposition]\label{def:bilateral-proposition}
	...
	\begin{itemize}
		...
		\item \textbf{Fusion closure for falsifiers}: $\fusion{s}{t} \in V$ for any $s, t \in F$
	\end{itemize}
\end{definition}
