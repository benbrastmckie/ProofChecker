...
\begin{definition}\label{def:constitutive-model}
	...
	\begin{itemize}
		...
		\item \textbf{n-place predicates} $\interp{F} = \langle \verifiertype{F}, \falsifiertype{F} \rangle$ where:
		      \begin{itemize}
			      \item $\verifiertype{F} = (\mathtt{Fin}\;n \to \statespace) \to \statespace$ is the \textit{verifier function type}
			      \item $\falsifiertype{F} = (\mathtt{Fin}\;n \to \statespace) \to \statespace$ is the \textit{falsifier function type}
		      \end{itemize}
