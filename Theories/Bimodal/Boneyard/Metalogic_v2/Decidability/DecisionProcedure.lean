...
The FMP provides explicit bounds on the search space, ensuring termination.
...
-/
def isValidFormula (φ : Formula) : Bool :=
  (decide φ).isValid
...
def isSatisfiable (φ : Formula) : Bool :=
  ¬isValidFormula φ.neg

/--
Decide with automatic FMP-based fuel based on formula complexity.

Uses the FMP bounds from Metalogic_v2 to ensure sufficient fuel
for termination while avoiding unnecessary computation.
-/
...
/--
Decide with explicit FMP-based fuel.

This uses the theoretical bound from FMP: 2^|closure(φ)| * 2 steps
is sufficient to explore all possible tableau branches.
-/
def decideWithFMPFuel (φ : Formula) : DecisionResult φ :=
  decide φ 10 (fmpBasedFuel φ)
