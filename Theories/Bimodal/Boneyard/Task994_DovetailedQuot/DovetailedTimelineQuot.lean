...
theorem forward_F_core (p : DovetailedPoint)
  ...
  · -- Small step case: use density to find a formula with larger encoding
    ...
    | succ j' =>
      ...
      -- We want: ∃ q, ExistsTask p.mcs q.mcs ∧ psi ∈ q.mcs
      -- We have: F(iteratedFuture j' psi) ∈ w.mcs, ExistsTask p.mcs w.mcs
      --
      -- If we can find q' with ExistsTask w.mcs q'.mcs ∧ iteratedFuture j' psi ∈ q'.mcs,
      -- then by transitivity ExistsTask p.mcs q'.mcs.
