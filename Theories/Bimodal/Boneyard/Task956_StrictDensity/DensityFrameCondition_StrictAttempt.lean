/-!
# Archived: Strict Density Frame Condition Attempts

This file contains archived code from Task 956 - attempts to prove strict density
at the MCS level. These attempts are structurally blocked due to the h_indep problem.

See README.md in this directory for full explanation.

Archived: 2026-03-13
Original location: Theories/Bimodal/Metalogic/StagedConstruction/DensityFrameCondition.lean
Lines archived: 271-2559 (2289 lines)
Sorry count: 26

Note: This file will NOT compile independently. It requires the non-archived
portions of DensityFrameCondition.lean and proper import setup.
-/

-- Original code below (lines 271-2559)
-- ==================================
            ...
            · -- W ~ M' as well, still not strict from M' side
              ...
              | inr h_F_delta_M' =>
                ...
                have h_GG_phi_M : Formula.all_future (Formula.all_future phi) ∈ M :=
                  set_mcs_implication_property h_mcs (theorem_in_mcs h_mcs h_T4_phi) h_G_phi_M
                ...
                -- Since G(phi) ∈ M and phi ∉ M, we have F(neg(phi)) ∈ M (by not_G_implies_F_neg? No, G(phi) ∈ M)
                ...
                · -- G(chi) ∈ M: chi ∈ GContent(M) ⊆ W (by h_R_MW)
                  ...
                  -- The key insight: we're stuck in a loop because W ~ M' ~ V
                ...
                · -- G(chi) ∉ M: Case A with chi
                  have h_F_neg_chi : Formula.some_future (Formula.neg chi) ∈ M :=
                    not_G_implies_F_neg h_mcs h_G_chi_M
                  obtain ⟨Y₁, h_Y₁_mcs, h_R_MY₁, h_F_neg_Y₁⟩ :=
                  ...
                  · -- CanonicalR Y M'
                    ...
                    · -- Y ~ M: use strict_density_M_reflexive (but M is not reflexive!)
                      ...
                      -- No, chi.neg ∈ Y doesn't mean G(chi.neg) ∈ Y
                      -- But if Y sees M and M sees Y, they share GContent modulo T4
                      -- The key: since M is not reflexive, some phi has G(phi) ∈ M, phi ∉ M
                      -- If Y sees M (h_YM), G(phi) ∈ M implies... wait, h_YM is GContent(Y) ⊆ M
                      -- So phi ∈ GContent(Y) → phi ∈ M. But phi ∉ M.
                      -- So phi ∉ GContent(Y), meaning G(phi) ∉ Y
                      -- But G(G(phi)) ∈ M (by T4), so G(phi) ∈ GContent(M) ⊆ Y (by h_R_MY)
                      -- So G(phi) ∈ Y! Contradiction with G(phi) ∉ Y
                      exfalso
                      have h_GG_phi_M' : Formula.all_future (Formula.all_future phi) ∈ M :=
                        set_mcs_implication_property h_mcs (theorem_in_mcs h_mcs h_T4_phi) h_G_phi_M
                      have h_G_phi_Y : Formula.all_future phi ∈ Y := h_R_MY h_GG_phi_M'
                      have h_phi_M_via_Y : phi ∈ M := h_YM h_G_phi_Y
                  ...
                  · -- Y = M'
                    ...
                    have h_not_M'Y₁ : ¬CanonicalR M' Y₁ := by
                      ...
                      have h_GG_chi_M' : Formula.all_future (Formula.all_future chi) ∈ M' :=
                        set_mcs_implication_property h_mcs' (theorem_in_mcs h_mcs' h_T4_chi) h_G_chi_M'
                      have h_G_chi_Y₁ : Formula.all_future chi ∈ Y₁ := h_M'Y₁ h_GG_chi_M'
                    ...
                    · -- Y₁ ~ M: same contradiction as Y ~ M case
                      exfalso
                      have h_G_phi_Y₁ : Formula.all_future phi ∈ Y₁ := h_R_MY₁ h_GG_phi_M
                      have h_phi_M_via_Y₁ : phi ∈ M := h_Y₁M h_G_phi_Y₁
                      exact h_phi_not_M h_phi_M_via_Y₁
                    · -- Y₁ is strict!
