...
This is a port from `Bimodal.Metalogic.Decidability.Tableau` with minimal changes.
The tableau rules are logic-specific and do not depend on the representation
architecture, so they are essentially unchanged.
