...
They abstract from universal laws governing transitions between world states while retaining temporal duration.

\begin{definition}[Task Frame]
A \textbf{task frame} over temporal type $T$ is a triple $\taskframe = (\worldstate, T, {\taskto{}})$ where:
\begin{itemize}
  \item $\worldstate$ is a type of \textbf{world states}---complete instantaneous configurations of the system
  \item $T$ is the \textbf{temporal duration type} with totally ordered abelian group structure
  \item $w \taskto{x} u$ is the \textbf{task relation}: world state $w$ can transition to $u$ with duration $x$
\end{itemize}
