...
\begin{theorem}[Box-Conjunction]
$\proves \nec(\varphi \land \psi) \leftrightarrow (\nec\varphi \land \nec\psi)$
\end{theorem}

\begin{theorem}[Diamond-Disjunction]
$\proves \poss(\varphi \lor \psi) \leftrightarrow (\poss\varphi \lor \poss\psi)$
\end{theorem}

\begin{theorem}[S5 Diamond-Box to Truth]
$\proves \poss\nec\varphi \imp \varphi$
\end{theorem}

\begin{theorem}[T-Box Consistency]
$\proves \nec(\varphi \land \lneg\varphi) \imp \falsum$
\end{theorem}
...
% TODO: there should also be a theory for the H operator, or perhaps just a corollary of the theorem above. This should be included in the Lean source code as well if it isn't already.
...
\texttt{ModalS5.lean} & S5 characteristic theorems (24 theorems) \\
