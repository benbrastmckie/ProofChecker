...
An atom $p$ is true at world state $[h, t]$ iff $p \in \text{mcs}(h(t))$.
\end{definition}

\begin{lemma}[Truth Lemma]
In the semantic canonical model, $\varphi \in \text{mcs}(h(t))$ iff $\varphi$ is true at world state $[h, t]$.\footnote{This is proven as \texttt{semantic\_truth\_lemma\_v2}.}
\end{lemma}
...
If $\context \satisfies \varphi$ then $\context \proves \varphi$.{This is proven as \texttt{main\_strong\_completeness} with bridge sorries for the general valid connection.}
\end{theorem}

\begin{theorem}[Provable iff Valid]
$\proves \varphi$ iff $\valid{\varphi}$.
This is proven as \texttt{main\_provable\_iff\_valid}, establishing the completeness of the proof system.
\end{theorem}

\begin{definition}[Finite Model Property]
If a formula is satisfiable, it is satisfiable in a finite model bounded by the formula's modal and temporal depth.
This is stated as \texttt{finite\_model\_property}.
\end{definition}

\subsection{Proof Strategy}

The semantic approach defines world states as equivalence classes of (history, time) pairs.
Two pairs $(h_1, t_1)$ and $(h_2, t_2)$ are equivalent iff $h_1(t_1) = h_2(t_2)$ as underlying world states.
