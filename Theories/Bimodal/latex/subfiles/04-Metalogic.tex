...
% FIX: make negation complete a definition and then state that MCSs are negation complete.

Maximal consistent sets (MCS) have the key property that they are \textbf{negation-complete}:
...
% FIX: it feels discontinuous to have both a footnote with a reference and then an explanation on the same thing. Better to move the content of the footnote above into the following explanation. Also, the connection between sets and contexts is left unexplained which needs to be addressed to connect the dots for readers while keeping explanations brief and to the point.
...
The Representation Theorem provides the fundamental bridge between syntactic consistency and semantic satisfiability.
The subsequent completeness theorems follow directly from this result.

\subsubsection{Canonical World States}

% FIX: This is confusing since it should introduce how histories and times are defined FIRST, and then use these to define canonical world states. In general, no definition should be stated in terms of definitions not yet given.
...
% FIX: define the quotient construction to avoid empty references that leave the reader unable to follow.
...
% NOTE: reference the lean theorem names in the theorem itself. For instance, replace 'Representation Theorem' below with '\texttt{representation\_theorem}' and remove the footnote. This will help guide the reader to the lean name of the theorem, pairing it with the latex name of the theorem which uses section numbers and theorem numbers automatically incremented
...
The elegance of this approach is that the MCS construction makes the truth lemma essentially trivial since truth \emph{is} membership in the MCS.

% FIX: the following stronger theorem also needs some explanation following
...
The following diagram illustrates how the core infrastructure feeds into the Representation Theorem, from which completeness follows as a corollary.

% NOTE: dependency diagrams should always have the more fundamental results come above (earlier in the order of what the reader sees), with the results following below. In particular, the following diagram should be flipped with the core infrastructure coming first (above) and the central result and corollaries coming after (below)

% FIX: I don't need the 'Corollaries', 'Central Result', and 'Core Infrastructure' labels since instead it would be better to explain this following the diagram. It would be useful to have a diagram reference label that can be referred to where convenient.
...
\begin{tikzpicture}[
  ...
  % Core Infrastructure layer (bottom)
  \node[box, fill=blue!10] (deduct) at (0,0) {Deduction Theorem\\{\scriptsize\texttt{deduction\_theorem}}};
  \node[box, fill=blue!10] (linden) at (4.5,0) {Lindenbaum's Lemma\\{\scriptsize\texttt{set\_lindenbaum}}};
  \node[box, fill=blue!10] (mcs) at (9,0) {Maximal Consistent Sets\\{\scriptsize\texttt{MaximalConsistent}}};
  ...
  % Completeness layer (top)
  \node[box, fill=orange!15] (weak) at (1.5,4) {Weak Completeness\\{\scriptsize\texttt{semantic\_weak\_completeness}}};
  \node[box, fill=orange!15] (strong) at (7.5,4) {Strong Completeness\\{\scriptsize\texttt{main\_strong\_completeness}}};

  % Arrows from Core to Representation
  \draw[arrow] (deduct.north) -- ++(0,0.4) -| (repr.south west);
  \draw[arrow] (linden.north) -- (repr.south);
  \draw[arrow] (mcs.north) -- ++(0,0.4) -| (repr.south east);

  % Arrows from Representation to Completeness
  \draw[arrow] (repr.north) -- ++(0,0.4) -| (weak.south);
  \draw[arrow] (repr.north) -- ++(0,0.4) -| (strong.south);

  % Layer labels
  \node[anchor=east, font=\footnotesize\itshape] at (-1.5,0) {Core Infrastructure};
  \node[anchor=east, font=\footnotesize\itshape] at (-1.5,2) {Central Result};
  \node[anchor=east, font=\footnotesize\itshape] at (-1.5,4) {Corollaries};
\end{tikzpicture}
\end{center}
...
% FIX: some explanation is in order here to explain that this shows that the proof system and semantics align perfectly, capturing all logical consequences in the proof system without deriving anything that fails to be a logical consequence.
...
% TODO: the following syntactic approach would be much better if is possible to port over from the Boneyard. This is deserving of careful study to implement completely. By contrast, the semantic approach abandons the intended spirit of the representation theorem
...
% FIX: I don't like how short this next subsection is. Instead, its content should be included in the following Decidability subsection.
...
% FIX: It is not clear what it is for the decision procedure returns ``valid'' with proof $\pi$. This needs to be defined accurately rather than characterized vaguely. 
...
% FIX: it is also important to explain what 'Try direct axiom proof' actually amounts to here in the context of using LEAN 4, and similarly for the proof search with limited depth, explaining these concepts to users unfamiliar with LEAN 4 while being brief and concise. Also briefly explain what a tableau is for users unfamiliar with logic. The following section can then provide more concrete details, but some indication here would also be helpful to explain what the goal is and why it works.
...
% FIX: some explanation would help readers here make sense of what these values mean
...
% FIX: further explanation of why this is useful despite the computational limitations
...
The Metalogic\_v2 directory contains 27 Lean files organized into six subdirectories.
...
\begin{itemize}
  \item \textbf{Core/}: Foundational definitions including provability (\texttt{ContextDerivable}), deduction theorem, and maximal consistent sets.
  \item \textbf{Soundness/}: Validity proofs for all 15 axiom schemata and the main soundness theorem.
  \item \textbf{Representation/}: Canonical model construction, truth lemma, and the central representation theorems.
  \item \textbf{Completeness/}: Weak and strong completeness theorems derived from representation.
  \item \textbf{Applications/}: Compactness theorem (trivial for list-based contexts).
  \item \textbf{Decidability/}: Tableau-based decision procedure with proof/countermodel extraction.
\end{itemize}
...
The Metalogic\_v2 codebase has three remaining \texttt{sorry} statements, none of which block the core completeness results:
