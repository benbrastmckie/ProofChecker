...
**Irreflexive Temporal Semantics**: As of Task #956, temporal operators G (all_future)
and H (all_past) use IRREFLEXIVE semantics (< instead of ≤), meaning "strictly future/past"
rather than "now and future/past". This resolves the T-axiom obstruction for density proofs.
...
- `M,τ,x ⊨ Past φ` iff `M,τ,y ⊨ φ` for all y ∈ D where y < x (past, irreflexive)
- `M,τ,x ⊨ Future φ` iff `M,τ,y ⊨ φ` for all y ∈ D where x < y (future, irreflexive)
...
✓ Past: `∀ (s : D), s < t → truth_at M τ s φ`
  uses irreflexive ordering (strictly past) for density proofs
✓ Future: `∀ (s : D), t < s → truth_at M τ s φ`
  uses irreflexive ordering (strictly future) for density proofs
...
- Temporal past/future quantify over ALL strictly past/future times in D (not just domain)
...
- Past (P): true iff φ true at all strictly past times in D (not just domain)
- Future (F): true iff φ true at all strictly future times in D (not just domain)
...
/--
Truth of past: formula true at all strictly past times (irreflexive).
-/
theorem past_iff
    {D : Type*} [AddCommGroup D] [LinearOrder D] [IsOrderedAddMonoid D]
    {F : TaskFrame D} {M : TaskModel F} {τ : WorldHistory F}
...
/--
Truth of future: formula true at all strictly future times (irreflexive).
-/
theorem future_iff
    {D : Type*} [AddCommGroup D] [LinearOrder D] [IsOrderedAddMonoid D]
    {F : TaskFrame D} {M : TaskModel F} {τ : WorldHistory F}
