...
**Reflexive Temporal Semantics**: As of Task #967, temporal operators G (all_future)
and H (all_past) use REFLEXIVE semantics (≤ instead of <), meaning "now and all future/past"
rather than "strictly future/past". This enables T-axiom validity (G(φ) → φ, H(φ) → φ),
which completes the Gabbay IRR proof for canonicalR_irreflexivity.
...
✓ Past: `∀ (s : D), s ≤ t → truth_at M τ s φ`
  uses reflexive ordering (present and past) enabling T-axiom validity
✓ Future: `∀ (s : D), t ≤ s → truth_at M τ s φ`
  uses reflexive ordering (present and future) enabling T-axiom validity
...
- Temporal past/future quantify over ALL past/future times in D (reflexive, not just domain)
...
- Past (P): true iff φ true at current time and all past times in D (reflexive)
- Future (F): true iff φ true at current time and all future times in D (reflexive)
...
/--
Truth of past: formula true at current time and all past times (reflexive).
-/
theorem past_iff
    {D : Type*} [AddCommGroup D] [LinearOrder D] [IsOrderedAddMonoid D]
    {F : TaskFrame D} {M : TaskModel F} {τ : WorldHistory F}
...
/--
Truth of future: formula true at current time and all future times (reflexive).
-/
theorem future_iff
    {D : Type*} [AddCommGroup D] [LinearOrder D] [IsOrderedAddMonoid D]
    {F : TaskFrame D} {M : TaskModel F} {τ : WorldHistory F}
...
theorem truth_double_shift_cancel (M : TaskModel F) (Omega : Set (WorldHistory F))
  ...
  | all_past ψ ih =>
    ...
    constructor
    · intro h s h_le
      exact (ih s).mp (h s h_le)
    · intro h s h_le
      exact (ih s).mpr (h s h_le)
  | all_future ψ ih =>
    simp only [truth_at]
    constructor
    · intro h s h_le
...
theorem time_shift_preserves_truth (M : TaskModel F) (Omega : Set (WorldHistory F))
  ...
  | all_past ψ ih =>
    -- Past quantifies over present and past times (reflexive)
