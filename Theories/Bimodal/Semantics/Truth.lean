...
✓ Past: `∀ (s : D), s < t → truth_at M τ s φ`
  uses strict ordering (all strictly past times)
✓ Future: `∀ (s : D), t < s → truth_at M τ s φ`
  uses strict ordering (all strictly future times)
...
- Temporal past/future quantify over ALL strictly past/future times in D (strict, not just domain)
...
- Past (H): true iff φ true at all strictly past times in D (strict)
- Future (G): true iff φ true at all strictly future times in D (strict)
...
/--
Truth of past: formula true at all strictly past times (strict).
-/
theorem past_iff
    {D : Type*} [AddCommGroup D] [LinearOrder D] [IsOrderedAddMonoid D]
    {F : TaskFrame D} {M : TaskModel F} {τ : WorldHistory F}
...
/--
Truth of future: formula true at all strictly future times (strict).
-/
theorem future_iff
    {D : Type*} [AddCommGroup D] [LinearOrder D] [IsOrderedAddMonoid D]
    {F : TaskFrame D} {M : TaskModel F} {τ : WorldHistory F}
