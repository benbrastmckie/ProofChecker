...
theorem valid_iff_empty_consequence (φ : Formula) :
    (⊨ φ) ↔ ([] ⊨ φ) := by
  constructor
  · intro h T _ _ _ F M τ t ht _
    exact h T F M τ t ht
  · intro h T _ _ _ F M τ t ht
    exact h T F M τ t ht (by intro ψ hψ; exact absurd hψ List.not_mem_nil)
...
theorem consequence_monotone {Γ Δ : Context} {φ : Formula} :
    Γ ⊆ Δ → (Γ ⊨ φ) → (Δ ⊨ φ) := by
  intro h_sub h_cons T _ _ _ F M τ t ht h_delta
...
theorem valid_consequence (φ : Formula) (Γ : Context) :
    (⊨ φ) → (Γ ⊨ φ) :=
  fun h T _ _ _ F M τ t ht _ => h T F M τ t ht
...
theorem consequence_of_member {Γ : Context} {φ : Formula} :
    φ ∈ Γ → (Γ ⊨ φ) := by
  intro h _ _ _ _ F M τ t ht h_all
  exact h_all φ h
...
theorem unsatisfiable_implies_all {Γ : Context} {φ : Formula} :
  ...
  fun h_unsat T _ _ _ F M τ t ht h_all =>
