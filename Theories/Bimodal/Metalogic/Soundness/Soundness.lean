...
Soundness states that every derivable formula is semantically valid:
if Gamma derives phi, then Gamma semantically entails phi.

## Main Results

- `soundness`: Derivability implies semantic validity (`Gamma derives phi -> Gamma entails phi`)
- `axiom_valid`: All 15 TM axioms are valid
...
/--
Propositional K axiom is valid: `entails (phi -> (psi -> chi)) -> ((phi -> psi) -> (phi -> chi))`.
-/
...
/--
Propositional S axiom is valid: `entails phi -> (psi -> phi)`.
-/
...
/--
Modal T axiom is valid: `entails Box phi -> phi`.
-/
...
/--
Modal 4 axiom is valid: `entails Box phi -> Box Box phi`.
-/
...
/--
Modal B axiom is valid: `entails phi -> Box Diamond phi`.
-/
...
/--
Modal 5 Collapse axiom is valid: `entails Diamond Box phi -> Box phi`.
-/
...
/--
EFQ axiom is valid: `entails falsity -> phi`.
-/
...
/--
Peirce's Law is valid: `entails ((phi -> psi) -> phi) -> phi`.
-/
...
/--
Modal K Distribution axiom is valid: `entails Box(phi -> psi) -> (Box phi -> Box psi)`.
-/
...
/--
Temporal K Distribution axiom is valid: `entails G(phi -> psi) -> (G phi -> G psi)`.
-/
...
/--
Temporal 4 axiom is valid: `entails G phi -> G G phi` (reflexive semantics: <=).
-/
...
/--
Temporal A axiom is valid: `entails phi -> G(sometime_past phi)`.
-/
...
/--
TL axiom validity: `Always phi -> G(H phi)` is valid in all task semantic models (reflexive semantics: <=).
-/
theorem temp_l_valid (φ : Formula) :
  ...
  -- Need to show: truth_at M tau r phi, given hrs : r <= s and hts : t <= s
  by_cases h_rt : r ≤ t
  · exact h_past r h_rt
...
/--
MF axiom validity: `Box phi -> Box(G phi)` is valid in all task semantic models.
-/
...
/--
TF axiom validity: `Box phi -> G(Box phi)` is valid in all task semantic models.
-/
...
theorem axiom_valid {φ : Formula} : Axiom φ → ⊨ φ := by
  ...
  | temp_t_future ψ =>
    -- G phi -> phi: if phi holds at all times s >= t, then phi holds at t (reflexivity: t <= t)
    intro T _ _ _ F M τ t; unfold truth_at; intro h; exact h t (le_refl t)
  | temp_t_past ψ =>
    -- H phi -> phi: if phi holds at all times s <= t, then phi holds at t (reflexivity: t <= t)
    intro T _ _ _ F M τ t; unfold truth_at; intro h; exact h t (le_refl t)
...
If `Gamma derives phi` (phi is derivable from context Gamma), then `Gamma entails phi` (phi is a semantic consequence of Gamma).
-/
theorem soundness (Gamma : Context) (φ : Formula) : (Gamma ⊢ φ) → (Gamma ⊨ φ) := by
  intro h_deriv
  ...
  | @assumption Gamma' φ' h_mem =>
    intro T _ _ _ F M τ t h_all
    exact h_all φ' h_mem

  | @modus_ponens Gamma' φ' ψ' _ _ ih_imp ih_phi =>
    intro T _ _ _ F M τ t h_all
  ...
  | @weakening Gamma' Delta' φ' _ h_sub ih =>
    intro T _ _ _ F M τ t h_all
    apply ih T F M τ t
    intro ψ h_mem
