...
- `GContent_subset_implies_HContent_reverse`: GContent(M) ⊆ M' implies HContent(M') ⊆ M
- `HContent_subset_implies_GContent_reverse`: HContent(M) ⊆ M' implies GContent(M') ⊆ M
...
theorem forward_temporal_witness_seed_consistent (M : Set Formula) (h_mcs : SetMaximalConsistent M)
  ...
  · -- Case: psi ∉ L, so L ⊆ GContent M
    ...
    have h_G_bot_in_M : Formula.all_future Formula.bot ∈ M :=
      set_mcs_closed_under_derivation h_mcs (Context.map Formula.all_future L)
        h_G_L_in_M d_G_bot
    ...
    have h_G_neg_psi : Formula.all_future (Formula.neg psi) ∈ M :=
      set_mcs_implication_property h_mcs (theorem_in_mcs h_mcs h_G_imp) h_G_bot_in_M
...
theorem past_temporal_witness_seed_consistent (M : Set Formula) (h_mcs : SetMaximalConsistent M)
  ...
  · -- Case: psi ∉ L, so L ⊆ HContent M
    ...
    have h_H_bot_in_M : Formula.all_past Formula.bot ∈ M :=
      set_mcs_closed_under_derivation h_mcs (Context.map Formula.all_past L)
        h_H_L_in_M d_H_bot
    ...
    have h_H_neg_psi : Formula.all_past (Formula.neg psi) ∈ M :=
      set_mcs_implication_property h_mcs (theorem_in_mcs h_mcs h_H_imp) h_H_bot_in_M
