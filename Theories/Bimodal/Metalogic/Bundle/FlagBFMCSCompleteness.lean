...
equals the root MCS.
-/
theorem FlagBFMCS_satisfies_root (B : FlagBFMCS) (φ : Formula)
    (h_mem : φ ∈ B.root.world) :
    satisfies_at B B.eval_flag B.eval_flag_mem B.eval_element φ := by
  rw [satisfies_at_iff_mem]
  -- B.eval_element is in B.eval_flag, and its MCS is B.root.world
  simp only [FlagBFMCS.eval_element, chainFMCS, chainFMCS_mcs]
  exact h_mem

/--
Any formula in the root MCS of canonicalFlagBFMCS is satisfied at its evaluation position.
-/
theorem canonicalFlagBFMCS_satisfies_root (M0 : CanonicalMCS) (φ : Formula)
    (h_mem : φ ∈ M0.world) :
    satisfies_at (canonicalFlagBFMCS M0)
      (canonicalFlagBFMCS M0).eval_flag
      (canonicalFlagBFMCS M0).eval_flag_mem
      (canonicalFlagBFMCS M0).eval_element φ :=
  FlagBFMCS_satisfies_root (canonicalFlagBFMCS M0) φ h_mem
...
2. Construct B = canonicalFlagBFMCS(M0)
...
theorem FlagBFMCS_completeness_set (S : Set Formula)
  ...
  -- Construct the FlagBFMCS
  let B := canonicalFlagBFMCS M0
  use B
  intro φ hφ
  -- φ ∈ S implies φ ∈ M0.world (by h_ext_sub)
  have h_mem : φ ∈ M0.world := h_ext_sub hφ
  exact FlagBFMCS_satisfies_root B φ h_mem
