...
The following sections were archived (2026-03-26) as they contained
sorry-bearing experimental code on a non-critical path:
...
These sections attempted a DeferralRestrictedMCS approach that required
consistency lemmas (boundary_resolution_set, augmented_seed) which could not
be proven without additional constraints. The sorry-free ParametricTruthLemma
and boxClassFamilies constructions provide the complete alternative.
-/
