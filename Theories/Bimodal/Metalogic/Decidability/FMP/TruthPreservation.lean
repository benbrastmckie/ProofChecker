...
Under reflexive semantics, the T-axiom `temp_t_future : Gφ → φ` is valid,
so this follows by MCS deductive closure.
-/
...
Under reflexive semantics, the T-axiom `temp_t_past : Hφ → φ` is valid,
so this follows by MCS deductive closure.
-/
