...
/-!
# CanonicalR Recovery from CanonicalTask

This module establishes relationships between the duration-agnostic `CanonicalR`
...
### Forward Direction (CanonicalTask -> CanonicalR)
- `canonicalTask_forward_MCS_implies_canonicalR`: MCS-annotated chain implies CanonicalR
- `Succ_chain_implies_canonicalR`: Plain Succ + CanonicalR transitivity

### Backward Direction (CanonicalR -> CanonicalTask)
...
- `canonical_forward_F_with_canonicalR`: F-forward with CanonicalR witness (re-export)
- `successor_from_seriality`: Successor existence (re-export)

## References

- Task 13 research report: 01_canonicalr-recovery-analysis.md
- CanonicalFrame.lean: CanonicalR definition and key theorems
...
/-!
## Phase 2: Forward Direction (CanonicalTask implies CanonicalR)

Every forward Succ-chain of length >= 1 implies CanonicalR. Since each Succ step
includes `g_content u ⊆ v` as its first condition (which IS CanonicalR), and
CanonicalR is transitive (via temp_4), the forward direction follows by induction.
...
`CanonicalR u u` (g_content u ⊆ u) does NOT hold in general. In particular,
`G phi in u` does NOT imply `phi in u`. Therefore the forward direction only
gives CanonicalR for chains of length >= 1 (where at least one Succ step occurs).
For n = 0, we get identity (u = v) instead.
-/

/--
Forward direction (MCS-annotated version): A forward MCS-chain implies CanonicalR
or identity.

For n = 0: u = v (identity).
For n >= 1: Each Succ step gives CanonicalR via its first condition, and
...
/--
Single Succ step implies CanonicalR. Convenience re-export.
-/
...
/--
Forward chain of length 1 implies CanonicalR.

When a chain has exactly 1 step, it is a single Succ, which directly gives CanonicalR.
-/
...
/--
Forward chain of any length >= 1 with MCS annotations implies CanonicalR.
...
Succ step occurred, giving CanonicalR u w for some w, and transitivity gives CanonicalR u v.
-/
...
/-!
## Phase 3: Backward Direction (CanonicalR implies CanonicalTask)

The backward direction states: if CanonicalR u v (i.e., g_content u ⊆ v),
then there exists a Succ-chain from u to v.

This is significantly harder because CanonicalR only guarantees g_content
...
/--
Backward direction: CanonicalR implies existence of a forward CanonicalTask chain.
...
The key difficulty is that CanonicalR (g_content u ⊆ v) does not directly
...
canonical model API based on CanonicalR.
...
Succ implies CanonicalR, and canonical_forward_G applies.
-/
...
/--
F-forward property with CanonicalR witness.

If F(psi) is in MCS M, then there exists an MCS W such that CanonicalR M W
...
proof, and one often needs to derive CanonicalR from them for use with existing
