...
/-! ## Derivable Implies Swap Valid

Main theorem: if a formula is derivable from empty context with a dense-compatible derivation,
then its swap is valid on dense frames.
-/

/--
**Main Theorem for Temporal Duality Soundness**:
If `⊢ φ` with a dense-compatible derivation, then `φ.swap` is valid on dense frames.

This is proven by induction on the derivation structure, using:
- `axiom_swap_valid` for axiom cases
- `mp_preserves_swap_valid`, `modal_k_preserves_swap_valid`, etc. for rule cases
- `truth_at_swap_swap` (involution) for nested temporal_duality

**Note**: The IRR case uses the same product frame construction as IRRSoundness.
For now, we mark it sorry pending IRRSoundness fixes.
-/
theorem derivable_implies_swap_valid [DenselyOrdered D] [Nontrivial D]
    {φ : Formula} (d : DerivationTree [] φ)
    (h_dc : d.isDenseCompatible) :
    is_valid D φ.swap_past_future := by
  match d with
  | .axiom _ φ' h_ax =>
    exact axiom_swap_valid φ' h_ax h_dc
  | .assumption _ _ h_in =>
    exact absurd h_in (List.not_mem_nil)
  | .modus_ponens _ φ' ψ' d1 d2 =>
    have ⟨h_dc1, h_dc2⟩ := h_dc
    exact mp_preserves_swap_valid φ' ψ' (derivable_implies_swap_valid d1 h_dc1)
                                        (derivable_implies_swap_valid d2 h_dc2)
  | .necessitation φ' d' =>
    exact modal_k_preserves_swap_valid φ' (derivable_implies_swap_valid d' h_dc)
  | .temporal_necessitation φ' d' =>
    exact temporal_k_preserves_swap_valid φ' (derivable_implies_swap_valid d' h_dc)
  | .temporal_duality φ' d' =>
    -- swap(swap(φ')) = φ'. The ih gives is_valid D φ'.swap.
    -- We need is_valid D φ'.swap.swap = is_valid D φ'.
    -- Use truth_at_swap_swap to show this.
    intro F M Omega h_sc τ h_mem t
    have h_ih := derivable_implies_swap_valid d' h_dc F M Omega h_sc τ h_mem t
    exact (truth_at_swap_swap M Omega τ t φ').mpr h_ih
  | .irr _ _ _ d' =>
    -- IRR case requires product frame construction (see IRRSoundness)
    -- For now, marked sorry pending IRRSoundness fixes
    sorry
  | .weakening _ _ _ d' h_sub =>
    -- Weakening from [] to Γ: need Γ = [] for this to make sense
    -- Since we have DerivationTree [] φ, the d must have been from [] to []
    -- This is a contradiction with h_sub which gives Γ' ⊆ Δ = []
    have h_empty : ∀ ψ ∈ ([] : Context), ψ ∈ [] := h_sub
    exact derivable_implies_swap_valid d' h_dc
