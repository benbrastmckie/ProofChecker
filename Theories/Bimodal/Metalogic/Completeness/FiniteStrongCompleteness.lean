...
Given Γ ⊨ φ:
1. Build impChain Γ φ = (ψ₁ → (ψ₂ → ... → (ψₙ → φ)...))
2. Show ⊨ impChain Γ φ
3. By weak completeness, ⊢ impChain Γ φ
...
- Weak completeness: `Bimodal.Metalogic.Completeness.WeakCompleteness`
...
Given Γ = [ψ₁, ..., ψₙ] and φ, builds the formula ψ₁ → (ψ₂ → ... → (ψₙ → φ)...).

This transforms a semantic consequence Γ ⊨ φ into a validity statement
⊨ impChain Γ φ, allowing us to apply weak completeness.
-/
...
/--
impChain is semantically equivalent to "if all of Γ hold, then φ holds".
...
Given a derivation of impChain Γ φ, unfold it to derive φ from Γ.
-/

/--
Given a derivation of impChain Γ φ from context Δ, derive φ from context (Γ ++ Δ).
...
theorem context_provable_iff_entails (Γ : Context) (φ : Formula) :
  ...
  · -- Completeness: semantic consequence implies derivable
    exact finite_strong_completeness Γ φ
