...
def classical_merge (P Q : Formula) : ⊢ (P.imp Q).imp ((P.neg.imp Q).imp Q) := by
  ...
  have contradiction_intro : ∀ A B : Formula,
    ...
    have step1 : [(A.imp B.neg), (A.imp B)] ⊢ A.neg :=
      Bimodal.Metalogic.deduction_theorem
    ...
    -- Apply deduction theorem: [] ⊢ (A → B) → ((A → ¬B) → ¬A)
    exact Bimodal.Metalogic.deduction_theorem
  ...
  have step1 : [(P.imp Q)] ⊢ (P.neg.imp Q).imp Q :=
    Bimodal.Metalogic.deduction_theorem [(P.imp Q)] (P.neg.imp Q) Q h_combined

  exact Bimodal.Metalogic.deduction_theorem [] (P.imp Q) ((P.neg.imp Q).imp Q) step1
...
def ni (Γ : Context) (A B : Formula) (h1 : (A :: Γ) ⊢ B.neg) (h2 : (A :: Γ) ⊢ B) : Γ ⊢ A.neg := by
  ...
  exact Bimodal.Metalogic.deduction_theorem Γ A Formula.bot h_bot
...
def ne (Γ : Context) (A B : Formula) (h1 : (A.neg :: Γ) ⊢ B.neg) (h2 : (A.neg :: Γ) ⊢ B) : Γ ⊢ A := by
  ...
  have h_neg_neg : Γ ⊢ A.neg.neg :=
    Bimodal.Metalogic.deduction_theorem Γ A.neg Formula.bot h_bot
  -- Apply DNE: ¬¬A → A
...
def bi_imp (A B : Formula) :
  ...
  have step1 : [(A.imp B)] ⊢ (B.imp A).imp ((A.imp B).and (B.imp A)) :=
    Bimodal.Metalogic.deduction_theorem [(A.imp B)] (B.imp A) _ h_in_ctx

  -- Apply deduction theorem: [] ⊢ (A → B) → ((B → A) → ((A → B) ∧ (B → A)))
  exact Bimodal.Metalogic.deduction_theorem [] (A.imp B) _ step1
...
noncomputable def de (Γ : Context) (A B C : Formula) (h1 : (A :: Γ) ⊢ C) (h2 : (B :: Γ) ⊢ C) :
  ...
  have ac : Γ ⊢ A.imp C :=
    Bimodal.Metalogic.deduction_theorem Γ A C h1

  -- Apply deduction theorem to get Γ ⊢ B → C
  have bc : Γ ⊢ B.imp C :=
    Bimodal.Metalogic.deduction_theorem Γ B C h2
