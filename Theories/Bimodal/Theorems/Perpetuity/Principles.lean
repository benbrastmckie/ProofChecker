...
noncomputable def future_k_dist (A B : Formula) :
  ...
  have step4 : [(A.imp B).all_future] ⊢ A.all_future.imp B.all_future := by
    exact Bimodal.Metalogic_v2.Core.deduction_theorem [(A.imp B).all_future] A.all_future B.all_future step3_reordered
  ...
  have step5 : [] ⊢ (A.imp B).all_future.imp (A.all_future.imp B.all_future) := by
    exact Bimodal.Metalogic_v2.Core.deduction_theorem [] (A.imp B).all_future (A.all_future.imp B.all_future) step4
