# Implementation Plan: Complete Context Refactor Plan Execution (v2)
...
- **Revision Reason**: Incorporated preflight/postflight workflow context from tasks 320 and 321 to ensure adequate (not excessive) representation of workflow standards in refactored context structure
...
7. **Missing Workflow Standards**: No consolidated documentation of preflight/postflight workflow patterns (tasks 320/321 context)
...
**New Context from Tasks 320/321**:
- ✅ Preflight/postflight workflow patterns defined (task 321 plan v3)
- ✅ Two-level error logging standard (CRITICAL vs NON-CRITICAL)
- ✅ Verification checkpoint pattern documented
- ✅ Atomic write pattern for status-sync-manager
- ✅ Git-only rollback strategy
- ✅ Timing-aware delegation instructions

**Recommended Approach**: Execute plan as-is with minor revisions for unintended changes validation and integration of preflight/postflight standards
...
8. **NEW**: Integrate preflight/postflight workflow standards from tasks 320/321
...
- Extensive preflight/postflight documentation (adequate representation only)
...
- **NEW**: Must adequately represent preflight/postflight patterns without over-representation
...
- [ ] **NEW**: workflows/preflight-postflight.md created (workflow standards from tasks 320/321)
...
7. **NEW**: Integrate preflight/postflight workflow standards adequately
...
6. **NEW**: Create extensive preflight/postflight documentation (adequate only)
...
**NEW Risk 4: Preflight/Postflight Context Imbalance**
- **Impact**: Over-representation or under-representation of workflow patterns
- **Probability**: Low
- **Mitigation**: Review tasks 320/321 plans, extract essential patterns only
- **Contingency**: Adjust workflows/preflight-postflight.md scope in Phase 2

### Medium-Risk Areas

**Risk 5: Context Loading Performance**
- **Impact**: Slower context loading
- **Probability**: Low
- **Mitigation**: Test loading time before and after
- **Contingency**: Optimize file organization

**Risk 6: Meta-Builder Integration**
...
**Objective**: Create critical architecture files FIRST before other changes, including preflight/postflight standards
...
3. **NEW: Create workflows/preflight-postflight.md** (30 minutes):
   - **Purpose**: Adequate representation of workflow standards from tasks 320/321
   - **Content** (extracted from task 320 plan v3 and task 321 plan v3):
     * Preflight timing requirements (status update BEFORE work begins)
     * Postflight timing requirements (status update and artifact linking BEFORE return)
     * Verification checkpoint pattern (delegate → wait → verify → proceed)
     * Two-level error logging (CRITICAL vs NON-CRITICAL)
     * Atomic write pattern (write to temp, rename atomically)
     * Git-only rollback strategy
     * Timing-aware delegation instructions template
     * Status marker conventions (reference to status-markers.md)
   - **Scope**: Essential patterns only, ~300-400 lines
   - **NOT included**: Detailed bug analysis, extensive examples, implementation details
   - **Cross-references**: 
     * orchestration/state-management.md (for atomic writes)
     * workflows/status-transitions.md (for status markers)
     * standards/error-handling.md (for error logging)

**Validation**:
- [ ] architecture.md created and comprehensive
- [ ] command-structure.md created and clear
- [ ] **NEW**: preflight-postflight.md created with adequate coverage
...
1. **Orchestration Directory** (2.5 hours):
   ...
   - MERGE state-management.md + artifact-management.md → orchestration/state-management.md
     ...
     * **NEW**: Reference atomic write pattern from workflows/preflight-postflight.md
...
3. **Standards Directory** (1.5 hours):
   ...
   - **NEW**: Update error-handling.md to reference two-level error logging from workflows/preflight-postflight.md
...
4. **Workflows Directory** (30 minutes):
   ...
   - **NEW**: Update status-transitions.md to reference workflows/preflight-postflight.md for timing requirements
   - RENAME review.md → workflows/review-process.md
   - **NEW**: ADD preflight-postflight.md (created in Phase 2)
   - **Result**: 5 files (includes 1 new from Phase 2)
...
- [ ] **NEW**: Cross-references to preflight-postflight.md added
- [ ] Total: 36 files in core/ (down from 48, includes 1 new preflight-postflight.md)
...
3. **Execute Test Matrix** (1.5 hours):
   ...
   | **NEW**: Preflight/postflight | /research 315 | Status updates correctly |
...
- [ ] **NEW**: Preflight/postflight patterns accessible
...
2. **Update Documentation** (1 hour):
   ...
   - **NEW**: Document workflows/preflight-postflight.md purpose and scope
...
3. **Create Git Commit** (15 minutes):
   ...
   git commit -m "task 314: complete context refactor - 48→36 files, add architecture docs and workflow standards"
   ```
...
1. **Regression Testing** (30 minutes):
   ...
   - **NEW**: Test preflight/postflight workflow patterns
...
2. **Meta-Builder Validation** (30 minutes):
   ...
   - **NEW**: Verify preflight-postflight.md accessible
...
- [ ] **NEW**: Workflow standards accessible and adequate
...
1. **Phase 2 Validation**:
   ...
   - **NEW**: Review preflight-postflight.md for adequate coverage (not excessive)
...
2. **Phase 3 Validation**:
   ...
   - **NEW**: Verify cross-references to preflight-postflight.md
...
1. **Functional Testing**:
   ...
   - **NEW**: Test preflight/postflight pattern accessibility
...
- [ ] All 36 files in correct locations (includes preflight-postflight.md)
...
- [ ] **NEW**: Workflow standards adequate (not excessive)
...
3. **NEW: workflows/preflight-postflight.md** (NEW):
   - Preflight/postflight workflow standards
   - ~300-400 lines
   - Adequate representation of tasks 320/321 context
   - Essential patterns only

4. **Refactored Context Structure**:
   - 36 files in 6 directories (includes preflight-postflight.md)
   - 25% reduction from 48 files
...
6. **NEW**: Preflight/postflight context imbalance (over/under representation)
...
|--------|--------|-------|--------|
| Total files | 48 | 36 | 36 (25% reduction) |
...
| Architecture docs | 0 | 3 | 3 (critical files) |
...
- [ ] **NEW**: Workflow standards adequately represented
...
3. **Time Constraints**: Requires 20-22 hours of focused work
...
5. **NEW**: Integrate preflight/postflight standards adequately (not excessively)

### Integration with Tasks 320/321

**Context Extracted** (adequate representation):
- Preflight timing requirements (status update BEFORE work)
- Postflight timing requirements (status/artifact update BEFORE return)
- Verification checkpoint pattern (delegate → wait → verify → proceed)
- Two-level error logging (CRITICAL vs NON-CRITICAL)
- Atomic write pattern (temp files + atomic rename)
- Git-only rollback strategy
- Timing-aware delegation instructions

**NOT Extracted** (avoid over-representation):
- Detailed bug analysis (bugs #1-#7)
- Extensive implementation details
- Multiple code examples
- Testing procedures
- Rollback procedures (covered in error-handling.md)

**Cross-References Created**:
- workflows/preflight-postflight.md ← orchestration/state-management.md (atomic writes)
- workflows/preflight-postflight.md ← workflows/status-transitions.md (status markers)
- workflows/preflight-postflight.md ← standards/error-handling.md (error logging)
