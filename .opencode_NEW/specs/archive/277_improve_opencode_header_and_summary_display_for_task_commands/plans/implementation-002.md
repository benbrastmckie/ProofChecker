# Implementation Plan: Improve OpenCode Header Display for Task Commands
...
- **Revision Reason**: Remove summary conclusion with task/command reference since OpenCode console header already displays this information

## Overview

### Problem Statement
The OpenCode system lacks consistent user-facing headers for commands. Users cannot easily identify which task or command is running from the console output.
...
2. A new command-output.md standard documenting the format

**Removed from scope** (compared to v1): Summary conclusions with task/command references are not needed since the header already provides this context.
...
- Update orchestrator Stage 5 to format headers
...
- Adding summary conclusions (redundant with header)

## Risks and Mitigations

### Risk 1: Context Window Impact
**Severity**: Low  
**Description**: Adding header increases output by ~10 tokens  
**Mitigation**: Total output still ~110 tokens, well under limits. Summary remains <100 tokens.
...
3. Document token limits and formatting rules
4. Provide examples for each command type:
   - Task-based: /research, /plan, /implement, /revise, /task
   - Direct: /todo, /errors, /review, /meta
5. Add references to orchestrator.md and subagent-return-format.md
6. Document that summary conclusions are NOT needed (header provides context)
...
- [ ] Explicitly states that summary conclusions are not needed
...
### Phase 3: Update Orchestrator Stage 5 (Header Formatting) [NOT STARTED]
...
- [ ] No conclusion formatting logic (removed from v1)
...
3. Verify for each command:
   ...
   - No conclusion is added (removed from v1)
...
- [ ] No conclusions added to summaries
...
- Verify no conclusions are added
...
2. `.opencode/agent/orchestrator.md` - Updated with header logic
...
3. Test that commands work without headers
4. Investigate and fix issues before re-implementing

### Contingency Plan
If orchestrator changes are too complex:
1. Implement simplified version (basic headers only)
2. Add advanced formatting in follow-up task
...
- [ ] 0 conclusions added to summaries (removed from v1)
...
- **No Conclusions**: Removed from v1 to avoid redundancy with header

### Changes from Version 1
- **Removed**: Summary conclusion formatting logic (Phase 3 in v1)
- **Removed**: Conclusion format specifications from command-output.md (Phase 1 in v1)
- **Reduced Effort**: From 4.5 hours to 3.5 hours (1 hour saved)
- **Simplified**: Phase 3 now only handles header formatting, not conclusions
- **Clarified**: command-output.md explicitly states conclusions are not needed
