...
<workflow_execution>
  <stage id="1" name="ParseAndValidate">
    ...
    <process>
      1. Parse task number and optional focus from $ARGUMENTS
         - $ARGUMENTS contains: "196" or "196 focus on proof strategy" or "105-107"
         - Extract first token as task_number
         - Validate is integer or range (N-M)
         - Check for focus prompt in remaining arguments
         - Extract focus prompt if present: focus_prompt = remaining tokens
         - If no focus prompt: focus_prompt = ""
      ...
      5. Validate task status allows research (no force flag needed for research)
         case "$status" in
           "completed")
             echo "Warning: Task $task_number already completed"
             echo "Recommendation: Research already done, but you can re-research with --focus"
             echo "To proceed: /research $task_number --focus 'specific aspect to re-research'"
             exit 1
             ;;
           "abandoned")
           ...
           "researching")
             ...
             echo "If this is a stale status: /task --sync $task_number to reset"
             echo "To override: /research $task_number --focus 'force restart'"
             exit 1
             ;;
           "not_started"|"planned"|"partial"|"revised")
  ...
  <stage id="1.5" name="Preflight">
    ...
    <process>
      ...
      This ensures status updates immediately when research starts.
    ...
    <process>
      ...
      2. Invoke target agent via task tool:
         task(
           ...
           prompt="Research task ${task_number}: ${description}. ${focus_prompt}",
  ...
  <stage id="3" name="ValidateReturn">
    ...
    <process>
      ...
      4. VALIDATION STEP 3: Validate Status Field
         ...
         - Check status is valid enum: researched, partial, failed, blocked
         - If status invalid:
           * Log error: "[FAIL] Invalid status: ${status}"
           * Log: "Valid statuses: researched, partial, failed, blocked"
      ...
      6. VALIDATION STEP 5: Validate Artifacts (CRITICAL - only if status=researched)
         - If status == "researched":
           a. Check artifacts array is non-empty
              - artifact_count=$(echo "$subagent_return" | jq '.artifacts | length')
              - If artifact_count == 0:
                * Log error: "[FAIL] Agent returned 'researched' status but created no artifacts"
                * Log error: "Error: Phantom research detected - status=researched but no artifacts"
...
```bash
/research TASK_NUMBER [FOCUS]
/research 196
/research 196 "focus on proof strategies"
/research 105-107  # Batch research
```

**Arguments:**
- **TASK_NUMBER**: Required - Task number to research
- **FOCUS**: Optional - Specific focus area for research
...
4. Routes to appropriate research agent based on task language
5. Agent conducts research (with optional focus guidance)
6. Creates research artifacts (reports, sources, findings)
7. Updates task status to [RESEARCHED] via status-sync-manager
...
|----------|-------|-------|
| lean | lean-research-agent | lean-search, mathlib-api, lean-lsp |
| general | general-research-agent | Web search, document analysis, file operations |

## Features

- **Focus Support**: Optional focus prompt for targeted research
- **Batch Support**: Can research multiple tasks sequentially (e.g., 105-107)
- **Status Tracking**: Two-phase status updates with atomic state management
- **Artifact Management**: Automatic linking and verification

See `.opencode/agent/lean-research-agent.md` and `.opencode/agent/general-research-agent.md` for details.