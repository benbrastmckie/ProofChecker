...
if ! source "${CLAUDE_PROJECT_DIR}/.claude/lib/core/unified-location-detection.sh" 2>&1; then
  echo "ERROR: Failed to source unified-location-detection.sh"
  echo "DIAGNOSTIC: Check library exists at: ${CLAUDE_PROJECT_DIR}/.claude/lib/core/unified-location-detection.sh"
  exit 1
fi
```
...
**See**: [unified-location-detection.sh](../../lib/core/unified-location-detection.sh) for implementation details.
