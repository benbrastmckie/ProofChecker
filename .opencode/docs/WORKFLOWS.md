...
This system is built around several key workflows that automate complex, multi-step tasks. These workflows are managed by the `orchestrator`.
...
-   **Process**: The orchestrator uses the `codebase` and `refactor` to scan the entire project.
-   **Tasks**:
...
-   **Process**: The orchestrator uses the `translator` to perform the conversion.
-   **Tasks**:
