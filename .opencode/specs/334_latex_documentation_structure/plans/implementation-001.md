...
**Task**: 334 - Create LaTeX documentation for Logos system mirroring layer structure

**Created**: 2026-01-08

**Status**: [NOT STARTED]

**Estimated Effort**: 28-40 hours (4-6 days)
...
- **Plan Version**: 1
- **Reports Integrated**:
  - research-001.md: Comprehensive analysis of source LaTeX structure and target documentation design
  - research-002.md: Supplemental research on Constitutive and Causal layers for future Layer 1 documentation
...
- Copy and adapt source LaTeX assets (formatting.sty, notation.sty, etc.)
- Create logos-notation.sty with Logos-specific macros
- Extract content from LogicNotes.tex for Layer 0 (Core TM logic)
- Generate 6 subfiles: Introduction, Syntax, Semantics, ProofSystem, Metalogic, Theorems
- Add standardized sections per requirement (syntax extensions, semantic frames, proof system extensions, metalogical properties)
- Add Lean cross-references to definitions
- Validate LaTeX compilation and formatting
- Create preparatory templates for future Layer 1 (Explanatory Extension)

**Out of Scope**:
- Implementation of Layer 1-3 operators in Lean (future work)
- Full content for Layer 1-3 documentation (templates only)
- PDF styling customization beyond source standards
- Translation to other formats (HTML, Markdown)
...
1. **Source Structure**: LogicNotes.tex contains 1452 lines covering propositional → modal → temporal → bimodal logic with ~400 lines of problem sets (27%)
2. **Content Mapping**: Clear 1-to-1 correspondence between source sections and Logos Core components
3. **Formatting Standards**: 5 custom style files (formatting.sty, notation.sty, proof_styles.sty, bib_style.bst, glossary.tex)
4. **Target Structure**: 6 subfiles for Layer 0 (00-Introduction through 05-Theorems) using LaTeX subfiles package
5. **Extraction Rules**: Include definitions, axioms, theorems; exclude problem sets, proofs, Fitch natural deduction

**From research-002.md**:

1. **Layer Architecture Clarification**: Constitutive and Causal operators are part of Layer 1 (Explanatory Extension), NOT Layer 0
2. **Semantic Progression**: Layers build incrementally with increasing semantic complexity
3. **Constitutive Operators**: Grounding (≤), essence (⊑), propositional identity (≡), relevance (≼)
4. **Causal Operator**: Causation (○→) with temporal productive character
5. **Future Documentation**: Preparatory templates for Layer 1 with content from LAYER_EXTENSIONS.md

**Key Recommendations**:
- Use subfile architecture for modularity and independent compilation
- Exclude Fitch natural deduction (not in Logos implementation)
- Add standardized sections to each subfile
- Include Lean cross-references for traceability
- Provide minimal explanation (1-2 sentences per definition)
- Create Layer 1 templates for future activation
...
**Objective**: Create target directory structure and copy source assets.

**Tasks**:

1.1. **Create Directory Structure** (30 minutes)
   - Create Documentation/LaTeX/ directory
   - Create subfiles/ subdirectory
   - Create assets/ subdirectory
   - Create bibliography/ subdirectory
   - Create build/ subdirectory (for compiled PDFs)

1.2. **Copy Source Assets** (1 hour)
   - Copy formatting.sty from source to assets/
   - Copy notation.sty from source to assets/
   - Copy bib_style.bst from source to assets/
   - Copy glossary.tex from source to assets/
   - Verify file integrity (checksums or size comparison)
   - Test asset loading in minimal LaTeX document

1.3. **Create logos-notation.sty** (2-3 hours)
   - Create new file assets/logos-notation.sty
   - Add package header and dependencies
   - Define task frame notation macros (taskframe, worldstateset, Tor, taskrel, worldhist)
   - Define task model notation macros (taskmodel, valuation)
   - Define truth and validity notation macros (truthat, TMmodels, TMproves)
   - Define Lean cross-reference macros (leansrc, leandef)
   - Define axiom notation macros (axiomK, axiomS, axiomMT, etc.)
   - Define perpetuity principle macros (Pone through Psix)
   - Test macros in minimal LaTeX document
   - Verify compatibility with notation.sty

1.4. **Create Main Document** (1 hour)
   - Create LogosReference.tex with document class and packages
   - Add title, author, date metadata
   - Include formatting.sty, notation.sty, logos-notation.sty
   - Add subfile package and structure
   - Add table of contents
   - Add bibliography configuration
   - Test compilation with empty subfiles

1.5. **Validation** (30 minutes)
   - Verify all directories created
   - Verify all assets copied successfully
   - Verify logos-notation.sty compiles
   - Verify main document compiles with empty subfiles
   - Document any LaTeX errors or warnings

**Deliverables**:
- Directory structure: Documentation/LaTeX/ with subdirectories
- Copied assets: formatting.sty, notation.sty, bib_style.bst, glossary.tex
- New file: assets/logos-notation.sty (200-300 lines)
- Main document: LogosReference.tex (100-150 lines)
- Validation report: Compilation success/failure

**Acceptance Criteria**:
- All directories exist
- All assets copied and verified
- logos-notation.sty compiles without errors
- Main document compiles successfully
- Empty subfiles load correctly
