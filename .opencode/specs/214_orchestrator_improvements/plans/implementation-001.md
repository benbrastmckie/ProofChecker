...
Complete three related improvements to orchestrator.md: (1) Address FIX comment on line 14 by removing historical comparisons and describing current system capabilities directly, (2) Apply XML styling consistent with error-diagnostics-agent.md for better maintainability, (3) Document AGENTS.md research findings showing it's for custom instructions, not agent definitions.
...
- CRITICAL: AGENTS.md is for custom instructions/rules, NOT agent definitions - renaming orchestrator.md to AGENTS.md would break the orchestrator
- XML styling is backward compatible with existing command workflows
- Total implementation: 2 hours 15 minutes (30 min rewrite + 1.5 hours XML styling + 15 min documentation)
...
## Phase 2: Apply XML Styling (1.75 hours = 1.5 hours implementation + 15 minutes documentation)
...
12. **Document AGENTS.md research finding (15 minutes)**
    - Add new "AGENTS.md Research Note" section after Related Documentation:
      ```markdown
      ---
      
      ## AGENTS.md Research Note
      
      **Question**: Would renaming orchestrator.md to AGENTS.md make the orchestrator the default agent on startup?
      
      **Answer**: NO. AGENTS.md is for custom instructions/rules per https://opencode.ai/docs/rules/, NOT for agent definitions.
      
      **Explanation**:
      - AGENTS.md purpose: Custom instructions that apply to all agents (system-wide rules)
      - orchestrator.md purpose: Agent definition with specific workflow and capabilities
      - Renaming orchestrator.md to AGENTS.md would:
        - Break the orchestrator (not recognized as agent definition)
        - Treat orchestrator workflow as system-wide instructions (incorrect)
        - Lose orchestrator functionality entirely
      
      **Conclusion**: Keep orchestrator.md as agent definition. Use AGENTS.md only for custom system rules if needed.
      
      **Alternative Methods for Default Agent**: Use OpenCode configuration or command-line flags (outside scope of this task).
      ```
...
- [ ] "AGENTS.md Research Note" section added documenting findings
...
2. All 11 XML tags applied: <context>, <role>, <task>, <context_allocation>, <delegation_registry>, <process_flow>, <step_N>, <helper_functions>, <error_handling>, <validation_checks>, <references>
3. AGENTS.md research documented explaining why renaming won't work
4. Backward compatibility maintained - orchestrator works with existing commands
5. File structure improved for maintainability

**Overall Acceptance Criteria Met**:
- [x] Research completed on AGENTS.md approach (research phase confirmed)
- [x] AGENTS.md approach researched - does NOT work (documented in research findings)
...
- [ ] AGENTS.md research findings documented in new section
...
**Documentation**: AGENTS.md research note prevents future confusion about file naming
...
| 2 | Document AGENTS.md research finding | 15 minutes |
| **Phase 2 Total** | **Apply XML Styling** | **1.75 hours** |
| **Total** | **All Phases** | **2.25 hours** |
...
- [ ] Phase 2: All 11 XML tags applied
- [ ] Phase 2: AGENTS.md research documented
