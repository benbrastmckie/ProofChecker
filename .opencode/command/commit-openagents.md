...
```bash
npm run test:general -- --smoke
npm run test:coder -- --smoke
...
- `agents` - Agent configuration changes (general, coder)
...
fix(agents): correct delegation logic in general
refactor(evals): split test-runner into modular components
test(evals): add smoke tests for general
