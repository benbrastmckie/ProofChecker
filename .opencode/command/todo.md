...
If no CLAUDE.md suggestions were found (no meta tasks or all had "none" action):
- Omit the "CLAUDE.md suggestions" section.
...
2. Read current `.opencode/CLAUDE.md` content
...
**Problem**: OpenCode Issue #1132 causes jq commands with `!=` operators to fail with `INVALID_CHARACTER` or syntax errors when OpenCode generates them inline.
...
**Reference**: See `.opencode/context/core/patterns/jq-escaping-workarounds.md` for comprehensive patterns.
