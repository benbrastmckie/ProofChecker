...
class StateValidator:
            ...
            if match:
                ...
                while j < len(lines) and not lines[j].startswith('###'):
                    ...
                    if line.startswith('- **Effort**:'):
                        ...
                        k = j + 1
                        while k < len(lines) and lines[k].strip() and not lines[k].startswith('---') and not lines[k].startswith('- **'):
    ...
    def _check_task_existence(self, todo_tasks: Dict, state_data: Dict):
        ...
        state_numbers = set(p['project_number'] for p in state_data.get('active_projects', []))
    ...
    def _check_metadata_consistency(self, todo_tasks: Dict, state_data: Dict):
        ...
        state_tasks = {p['project_number']: p for p in state_data.get('active_projects', [])}
    ...
    def _check_structure(self, todo_tasks: Dict, state_data: Dict):
        ...
        content = self.todo_path.read_text()
        required_sections = ['## High Priority Tasks', '## Medium Priority Tasks', '## Low Priority Tasks']
    ...
    def repair(self, auto: bool = False) -> int:
        ...
        for issue in fixable_issues:
            try:
                ...
                if "missing Description field in TODO.md" in issue.message:
                    self._fix_missing_description_in_todo(issue.task_number, todo_tasks, state_data)
                    fixed_count += 1
                elif "missing description field in state.json" in issue.message:
                    self._fix_missing_description_in_state(issue.task_number, todo_tasks, state_data)
                    fixed_count += 1
                    ...
                    self._fix_priority_mismatch(issue.task_number, todo_tasks, state_data)
                    fixed_count += 1
                elif "language mismatch" in issue.message:
                    self._fix_language_mismatch(issue.task_number, todo_tasks, state_data)
                    fixed_count += 1
                    ...
                    self._fix_missing_in_state(issue.task_number, todo_tasks, state_data)
                    fixed_count += 1
            except Exception as e:
        ...
        print("\nValidating repairs...")
        self.validate()
        remaining_issues = [i for i in self.issues if i.severity == Severity.CRITICAL or i.severity == Severity.WARNING]
    ...
    def _fix_missing_description_in_todo(self, task_num: int, todo_tasks: Dict, state_data: Dict):
    ...
    def _fix_missing_description_in_state(self, task_num: int, todo_tasks: Dict, state_data: Dict):
            ...
            if re.match(task_pattern, line):
                ...
                while j < len(lines) and not lines[j].startswith('###'):
                    if lines[j].strip().startswith('- **Status**:'):
                        ...
                        self.todo_path.write_text('\n'.join(lines))
                        print(f"✓ Updated status for task {task_num} to [{todo_status}]")
                        return
                    j += 1
                break
            ...
            if re.match(task_pattern, line):
                ...
                while j < len(lines) and not lines[j].startswith('###'):
                    if lines[j].strip().startswith('- **Priority**:'):
                        ...
                        self.todo_path.write_text('\n'.join(lines))
                        print(f"✓ Updated priority for task {task_num} to {state_priority}")
                        return
                    j += 1
                break
            ...
            if re.match(task_pattern, line):
                ...
                while j < len(lines) and not lines[j].startswith('###'):
                    if lines[j].strip().startswith('- **Language**:'):
                        ...
                        self.todo_path.write_text('\n'.join(lines))
                        print(f"✓ Updated language for task {task_num} to {state_language}")
                        return
                    j += 1
                break
    ...
    def _fix_missing_in_todo(self, task_num: int, state_data: Dict):
        ...
        # Format task entry
        title = state_task.get('project_name', f'task_{task_num}').replace('_', ' ').title()
...
def main():
    ...
    parser.add_argument(
        '--fix',
        action='store_true',
        help='Repair issues automatically'
    )
    parser.add_argument(
        '--auto',
        action='store_true',
    ...
    parser.add_argument(
        '--specs-dir',
        type=Path,
        default=Path('.opencode/specs'),
        help='Path to specs directory (default: .opencode/specs)'
    )
