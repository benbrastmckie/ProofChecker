...
- **CRITICAL**: The `summary` field in return object is METADATA, not an artifact
- Summary field contains brief description (<100 tokens) for orchestrator context
- Summary field is NOT read from a file - it's generated by subagent and returned directly
...
- [ ] summary field is metadata string, NOT artifact file path
...
2. Extract summary from return object (metadata, NOT file)
...
- **Summary is extracted from return object `summary` field**, NOT read from file
- Summary field contains brief metadata (<100 tokens) generated by subagent
- Summary is used for orchestrator context, NOT stored as separate artifact (except for /implement)
- For /implement: Summary artifact (file) exists in artifacts array, AND summary metadata in summary field
- For /research, /plan, /revise: NO summary artifact, only summary metadata in return object
...
- If yes: Verify summary artifact in artifacts array
- If no: Summary artifact NOT required (e.g., /research, /plan)
