...
<input_parameters>
  <required>
    <task_numbers>Array of task numbers to execute in batch</task_numbers>
  </required>
  <optional>
...
<delegation_context_handling>
  ...
  <on_routing>
    Before routing to task-executor or other subagents:
    ...
    2. Check for cycles: if "task-executor" in delegation_path and routing to task-executor:
       - Return cycle error instead of routing
    3. Prepare delegation context for subagent:
       - depth: delegation_depth + 1
       - path: delegation_path.append("batch-task-orchestrator")
       - session_id: use provided session_id or generate if not provided
    4. Pass updated delegation context to subagent
  </on_routing>
  
  <on_return>
    Include delegation context in return metadata:
    - delegation_depth: Current depth value
    - delegation_path: Full path including batch-task-orchestrator
  ...
  <error_handling>
    ...
    2. Return error with standardized format:
       {
         "status": "failed",
         "summary": "Cannot execute batch tasks - max delegation depth would be exceeded",
         "artifacts": [],
         "metadata": {
         ...
         "errors": [{
           ...
           "message": "Max delegation depth (3) would be exceeded by routing to task-executor",
           "code": "MAX_DEPTH_EXCEEDED",
           "recoverable": false
         }],
         "next_steps": "Simplify workflow to reduce delegation depth or execute tasks directly"
       }
    3. Do NOT route to subagent
    
    If delegation cycle detected:
    1. Log: "Delegation cycle detected: {delegation_path} → {target_agent}"
    2. Return cycle error with full path for debugging
    3. Do NOT route to subagent
  </error_handling>
</delegation_context_handling>
