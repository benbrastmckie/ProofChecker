...
cd evals/framework
npm run eval:sdk -- --agent=general --prompt-variant=llama --suite=smoke-test

# Test with specific model
npm run eval:sdk -- --agent=general --prompt-variant=llama --model=ollama/llama3.2 --suite=core-tests
...
```bash
# Switch to a variant
./scripts/prompts/use-prompt.sh --agent=general --variant=llama

# Restore default (canonical agent file)
./scripts/prompts/use-prompt.sh --agent=general --variant=default
```
...
│   ├── general.md            # General default (Claude-optimized)
│   └── coder.md            # Coder default
└── prompts/                     # Model-specific variants
    ├── README.md               # This file
    ├── general/              # General variants
...
npm run eval:sdk -- --agent=general --prompt-variant=llama --suite=core-tests

# Custom test pattern
npm run eval:sdk -- --agent=general --prompt-variant=llama --pattern="01-critical-rules/**/*.yaml"
```
...
```bash
cp .opencode/prompts/general/TEMPLATE.md .opencode/prompts/general/my-variant.md
```
...
cd evals/framework
npm run eval:sdk -- --agent=general --prompt-variant=my-variant --suite=smoke-test
```
...
```bash
# Run core test suite
npm run eval:sdk -- --agent=general --prompt-variant=my-variant --suite=core-tests
...
Update `.opencode/prompts/general/README.md` with:
...
See [general/README.md](general/README.md) for detailed test results.
...
```bash
# Create suite
cp evals/agents/general/config/smoke-test.json \
   evals/agents/general/config/my-suite.json

# Edit suite (add your tests)
# Validate suite
cd evals/framework && npm run validate:suites general

# Run with your variant
npm run eval:sdk -- --agent=general --prompt-variant=my-variant --suite=my-suite
```
...
# Test Qwen 2.5
npm run eval:sdk -- --agent=general --prompt-variant=llama --model=ollama/qwen2.5 --suite=core-tests
...
**Per-variant results** (`.opencode/prompts/general/results/llama-results.json`):
...
cd evals/framework
npm run eval:sdk -- --agent=general --prompt-variant=your-variant --suite=core-tests
...
- [General Variants](general/README.md) - General-specific docs
...
ls .opencode/prompts/general/*.md

# Check variant exists
npm run eval:sdk -- --agent=general --prompt-variant=your-variant --suite=smoke-test
```
...
npm run eval:sdk -- --agent=general --prompt-variant=llama --model=ollama/llama3.2
```
...
**Questions?** See [general/README.md](general/README.md) or open an issue.
