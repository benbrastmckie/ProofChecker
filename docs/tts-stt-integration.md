...
1. **TTS Notifications**: Claude Code announces completion via Piper TTS with WezTerm tab identification
...
When Claude Code finishes responding, the Stop hook triggers `tts-notify.sh` which:

1. Checks a 60-second cooldown to prevent notification spam
2. Detects the WezTerm tab number via `wezterm cli list`
3. Generates a message: "Tab N: Claude has finished"
4. Speaks the message using Piper TTS
...
| `TTS_COOLDOWN` | `60` | Seconds between notifications |
...
4. When Claude finishes, hear "Tab 2: Claude has finished"
...
4. When you hear "Tab 1: Claude has finished", switch back
