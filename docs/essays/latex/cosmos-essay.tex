...
The fundamental limitation is not intelligence, but verification.
...
Consider materials science: raw sand is refined into glass through controlled processes that remove impurities and create useful transparency.
...
Natural language provides raw materials---intuitions about conjunction, negation, necessity, possibility, time, causation, belief, and obligation.
...
Rather than describing how ``if'' behaves in English, formal logic stipulates how conditionals \textit{should} behave for systematic reasoning.
The resulting operators remain intelligible yet are precise enough to be verifiable.
...
The \href{https://github.com/benbrastmckie/ProofChecker}{\texttt{ProofChecker}} implements an axiomatic proof theory and recursive semantics for the Logos---a unified formal language with operators for reasoning about necessity, possibility, counterfactuals, time, sufficiency, causation, and more.
...
We have completed the first Lean 4 formalization of a bimodal logic for temporal-modal reasoning.
The system combines operators for necessity and possibility with operators for past and future into a unified framework.

Every axiom has been proven sound by a machine-checkable proof---verified by Lean's type checker, not human arguments that might contain errors.
...
The Logos enables AI systems to reason about plans under uncertainty.
...
Modal operators represent alternatives: ``this is possible,'' ``this is necessary.''
Together, these operators enable representing and comparing plans.

\subsection{Unlimited Verified Training Data}

Current AI systems are trained on finite corpora---expensive to annotate, limited in scope, rife with errors.
Axiom systems offer an alternative: they generate infinite theorems.

Each theorem comes with a \textit{proof receipt}, a machine-checkable demonstration that the inference is valid.
Invalid inferences can be refuted with \textit{countermodels}---explicit scenarios showing why the reasoning fails.
...
An AI system can provide a machine-checkable proof that its conclusion follows from the premises---a certificate of correctness you can verify without understanding all the details.
...
Applications span any domain requiring verifiable reasoning under uncertainty: medical planning with counterfactual reasoning about interventions, legal reasoning tracking beliefs and obligations, multi-agent coordination modeling what others believe and prefer.
...
The Logos has a layered architecture: applications import exactly the operators they need---temporal and counterfactual reasoning for planning, epistemic reasoning for uncertainty, normative reasoning for obligations.

The \href{https://github.com/benbrastmckie/ProofChecker}{\texttt{ProofChecker}} provides proofs verified in Lean 4; the \href{https://github.com/benbrastmckie/ModelChecker}{\texttt{ModelChecker}} finds countermodels via Z3.
