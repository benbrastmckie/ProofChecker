...
The buildSeedAux recursive builder, consistency infrastructure, and membership preservation.
-/
...
theorem foldl_addFormula_fam_preserves (phi : Formula) (ty : SeedEntryType) (timeIdx : Int)
...
-/
theorem List.mem_modify_iff {α : Type*} {l : List α} {i : Nat} {f : α → α} {x : α} :
    x ∈ l.modify i f ↔
...
-/
lemma addFormula_preserves_getFormulas_diff_fam
...
NOTE: API compatibility issues - marked sorry for now.
-/
lemma addFormula_preserves_getFormulas_diff_time
...
-/
lemma addFormula_preserves_mem_getFormulas_same
...
-/
lemma foldl_addFormula_fam_preserves_mem_general
...
theorem mem_getFormulas_after_createNewFamily
  ...
  | none =>
    ...
    · -- At the new entry position
      rename_i h_cond
      right
      -- Extract entry from the match in h_cond
      split at h_cond
      · have h_entry := Option.some.inj h_cond
        rw [← h_entry] at h_mem
        simp only [Set.mem_singleton_iff] at h_mem
        rename_i h_match
        simp only [Bool.and_eq_true, beq_iff_eq] at h_match
        exact ⟨h_mem, h_match.1.symm, h_match.2.symm⟩
      · simp only [List.find?_nil] at h_cond
        exact absurd h_cond (by contradiction)
    · -- Not at new entry position either - contradiction since find? on [] is none
      rename_i h_cond
      -- h_mem : phi ∈ entry✝.formulas but entry✝ came from find? on empty list
      -- h_cond was the negative split, entry came from the else branch (find? [] = none)
      -- The split was on the match, so in the false branch, h_mem should be from none
      exfalso
      -- After the split of h_mem on the match in the none branch of the outer find?,
      -- the match produced none which was rewritten, making h_mem : phi ∈ ∅
      revert h_mem h_cond
      simp only [List.find?_nil, beq_iff_eq, Bool.and_eq_true]
      intro h_cond h_mem
      exact Set.not_mem_empty phi h_mem
...
theorem addFormula_preserves_hasPosition (seed : ModelSeed) (famIdx : Nat) (timeIdx : Int)
...
theorem addFormula_hasPosition_backward (seed : ModelSeed) (famIdx : Nat) (timeIdx : Int)
    (phi : Formula) (ty : SeedEntryType) (fam' : Nat) (time' : Int)
  ...
  | some idx =>
    ...
    | inl h_other =>
      ...
      have h_mem : e ∈ seed.entries := List.mem_of_getElem? hj
    ...
    | inr h_modified =>
      ...
      -- h_eq says e = {familyIdx := orig.familyIdx, ...}
      -- h_orig : seed.entries[idx]? = some orig
      subst h_eq
      -- Now he_pred is about the modified entry's familyIdx/timeIdx = orig's
      simp only at he_pred
      right
      -- orig = seed.entries[idx] from h_orig
      have h_orig_eq : orig = seed.entries[idx] := by
        exact (List.getElem?_eq_some_iff.mp h_orig).2.symm
      rw [h_orig_eq] at he_pred
  ...
  | none =>
    ...
    | inr h_new =>
      ...
      exact ⟨h_new.1.symm, h_new.2.symm⟩

/-- addFormula creates the position it targets. -/
lemma addFormula_self_hasPosition (seed : ModelSeed) (famIdx : Nat) (timeIdx : Int)
    (phi : Formula) (ty : SeedEntryType) :
...
lemma createNewFamily_self_hasPosition (seed : ModelSeed) (timeIdx : Int) (phi : Formula) :
...
lemma createNewTime_self_hasPosition (seed : ModelSeed) (famIdx : Nat) (timeIdx : Int)
    (phi : Formula) :
...
theorem foldl_addFormula_fam_preserves_hasPosition (phi : Formula) (ty : SeedEntryType)
...
theorem foldl_addFormula_times_preserves_hasPosition (phi : Formula) (famIdx : Nat)
...
theorem addToAllFamilies_preserves_hasPosition (seed : ModelSeed) (timeIdx : Int)
...
theorem addToAllFutureTimes_preserves_hasPosition (seed : ModelSeed) (famIdx : Nat)
...
theorem addToAllPastTimes_preserves_hasPosition (seed : ModelSeed) (famIdx : Nat)
    (currentTime : Int) (phi : Formula) (fam' : Nat) (time' : Int)
    (h : seed.hasPosition fam' time') :
...
theorem createNewFamily_preserves_hasPosition (seed : ModelSeed) (timeIdx : Int)
    (phi : Formula) (fam' : Nat) (time' : Int) (h : seed.hasPosition fam' time') :
...
theorem createNewTime_preserves_hasPosition (seed : ModelSeed) (famIdx : Nat)
