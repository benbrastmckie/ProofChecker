...
with ExistsTask(M, M') and NOT ExistsTask(M', M), there exists W with
ExistsTask(M, W) AND ExistsTask(W, M').
...
1. From NOT ExistsTask(M', M), find distinguishing formula delta with
...
2. Case-split on G(delta) in M:
   - Case A (G(delta) not in M): F(neg(delta)) in M.
     ...
     with ExistsTask(M, W) and F(neg(delta)) in W. Then forward witness
     V from W with ExistsTask(W, V) and neg(delta) in V.
     Temporal linearity on M, V, M' gives three cases:
     * ExistsTask(V, M'): V is intermediate.
     * ExistsTask(M', V): delta in g_content(M') subset V but neg(delta)
       in V. Contradiction.
     * V = M': ExistsTask(W, V) = ExistsTask(W, M'), so W is intermediate.
   - Case B (G(delta) in M): Sub-split on ExistsTask(M', M'):
     * B1 (M' reflexive): Take W = M'. Both ExistsTask(M, M') and
       ExistsTask(M', M') hold.
     * B2 (M' not reflexive): g_content(M') is not a subset of M', so
       ...
       M' (by ExistsTask(M, M')), contradicting gamma not in M'. So
       G(gamma) not in M, and we apply Case A with gamma.

## Note on Strict Density

For STRICT density (where the intermediate W satisfies ¬ExistsTask(W, M) AND
¬ExistsTask(M', W)), see QuotientDensity.lean which proves this at the quotient
...
(under ExistsTask(M, M')), because this would place both delta and
neg(delta) in M'.
...
/--
In Case B (G(delta) in M with ExistsTask(M, M')), G(neg(delta)) is not in M.
...
Given:
- ExistsTask(M, M'), NOT ExistsTask(M', M)
- G(delta) in M', delta not in M
- F(neg(delta)) in M (Case A condition)

Then there exists W with ExistsTask(M, W) AND ExistsTask(W, M').

Strategy (double-density trick):
1. Density on F(neg(delta)): F(F(neg(delta))) in M. Get W_1 with
   ExistsTask(M, W_1) and F(neg(delta)) in W_1.
2. Forward witness from W_1 for neg(delta): Get V with
   ExistsTask(W_1, V) and neg(delta) in V.
3. ExistsTask(M, V) by transitivity.
4. Temporal linearity on M, V, M':
   - ExistsTask(M', V): delta in g_content(M') subset V, but
     neg(delta) in V. Contradiction.
   - ExistsTask(V, M'): V is intermediate.
   - V = M': ExistsTask(W_1, V) = ExistsTask(W_1, M'), so W_1 is intermediate.
-/
theorem density_frame_condition_caseA
    ...
    ∃ W : Set Formula, SetMaximalConsistent W ∧ ExistsTask M W ∧ ExistsTask W M' := by
  -- Step 1: Density on F(neg(delta)) gives F(F(neg(delta))) in M
  -- Then get W_1 with ExistsTask(M, W_1) and F(neg(delta)) in W_1
  ...
  -- F(neg(delta)) in W_1, so get V with ExistsTask(W_1, V) and neg(delta) in V
  obtain ⟨V, h_V_mcs, h_R_W₁V, h_neg_delta_V⟩ :=
    canonical_forward_F W₁ h_W₁_mcs (Formula.neg delta) h_F_neg_W₁
  -- Step 3: ExistsTask(M, V) by transitivity
  ...
  · -- ExistsTask(V, M'): V is the intermediate
    exact ⟨V, h_V_mcs, h_R_MV, h_VM'⟩
  · -- ExistsTask(M', V): delta in g_content(M') subset V, neg(delta) in V. Contradiction.
    exfalso
...
- **Case B** (G(delta) in M, delta not in M): Two sub-cases:
  - **B1** (ExistsTask(M', M') holds): Take W = M' directly. Both
    ExistsTask(M, M') (given) and ExistsTask(M', M') (sub-case) hold.
  - **B2** (ExistsTask(M', M') fails): Since g_content(M') is not a subset
    ...
    g_content(M) subset M' (by ExistsTask(M, M')), contradicting gamma
...
For all MCSs M, M' with ExistsTask(M, M') and NOT ExistsTask(M', M),
there exists an intermediate MCS W with ExistsTask(M, W) AND ExistsTask(W, M').
...
theorem density_frame_condition
  ...
  · -- Case B: G(delta) in M, delta not in M
    -- Sub-case split on whether M' is reflexive (ExistsTask(M', M')).
    by_cases h_R'_self : ExistsTask M' M'
    · -- Sub-case B1: ExistsTask(M', M') holds.
      -- Take W = M'. Then ExistsTask(M, M') (given) and ExistsTask(M', M') hold.
      exact ⟨M', h_mcs', h_R, h_R'_self⟩
    · -- Sub-case B2: ExistsTask(M', M') does not hold.
      ...
      -- Claim: G(gamma) ∉ M.
      -- Proof: If G(gamma) ∈ M, then gamma ∈ g_content(M) ⊆ M' (by ExistsTask(M, M')).
...
If M is not reflexive (¬ExistsTask M M), then the seriality witness W
satisfies ¬ExistsTask W M (strict future). This is because:
1. If ¬ExistsTask M M, then ∃ phi with G(phi) ∈ M and phi ∉ M
...
5. So g_content(W) ⊄ M, hence ¬ExistsTask W M
-/

/--
If M is not reflexive, then its seriality future W is strict: ¬ExistsTask W M.
-/
theorem irreflexive_SetMaximalConsistent.has_strict_future
    (M : Set Formula) (h_mcs : SetMaximalConsistent M)
    (h_not_refl : ¬ExistsTask M M) :
    ∃ W : Set Formula, SetMaximalConsistent W ∧ ExistsTask M W ∧ ¬ExistsTask W M := by
  -- Get the seriality witness
  obtain ⟨W, h_W_mcs, h_R_MW⟩ := SetMaximalConsistent.has_strict_future M h_mcs
  refine ⟨W, h_W_mcs, h_R_MW, ?_⟩
  -- Show ¬ExistsTask W M using the irreflexivity witness
...
intermediate: one that is not equivalent to the target (¬ExistsTask M' W).
...
/-- Strengthened Case A: when we have F(¬delta) ∈ M and G(delta) ∈ M',
    the density intermediate is STRICT: ¬ExistsTask(M', W).
    The strictness comes from ¬delta in V contradicting delta from g_content(M'),
    and the G/F duality making ExistsTask(M', W₁) impossible. -/
theorem density_frame_condition_caseA_strict
    ...
    (h_G_delta_M' : Formula.all_future delta ∈ M')
    (h_F_neg_delta : Formula.some_future (Formula.neg delta) ∈ M) :
    ∃ W : Set Formula, SetMaximalConsistent W ∧ ExistsTask M W ∧ ExistsTask W M' ∧
      ¬ExistsTask M' W := by
  -- Step 1: Density gives W₁ with ExistsTask(M, W₁) and F(¬delta) ∈ W₁
  obtain ⟨W₁, h_W₁_mcs, h_R_MW₁, h_F_neg_W₁⟩ :=
    density_of_canonicalR M h_mcs (Formula.neg delta) h_F_neg_delta
  -- Step 2: Forward witness gives V with ExistsTask(W₁, V) and ¬delta ∈ V
  obtain ⟨V, h_V_mcs, h_R_W₁V, h_neg_delta_V⟩ :=
    canonical_forward_F W₁ h_W₁_mcs (Formula.neg delta) h_F_neg_W₁
  -- Step 3: ExistsTask(M, V) by transitivity
  have h_R_MV : ExistsTask M V := canonicalR_transitive M W₁ V h_mcs h_R_MW₁ h_R_W₁V
  -- Key: ¬ExistsTask(M', V) because delta and ¬delta would coexist in V
  have h_not_M'V : ¬ExistsTask M' V := by
    intro h_R'V
    exact set_consistent_not_both h_V_mcs.1 delta (h_R'V h_G_delta_M') h_neg_delta_V
  -- Key: ExistsTask(M', W₁) is impossible by G/F duality
  ...
  have h_lin := canonical_forward_reachable_linear M V M' h_mcs h_V_mcs h_mcs' h_R_MV h_R
  rcases h_lin with h_VM' | h_M'V | h_eq
  · -- ExistsTask(V, M'): V is the strict intermediate
    exact ⟨V, h_V_mcs, h_R_MV, h_VM', h_not_M'V⟩
  · -- ExistsTask(M', V): impossible
    exact absurd h_M'V h_not_M'V
  · -- V = M': Use W₁ as intermediate instead
    -- W₁ has ExistsTask(M, W₁) and ExistsTask(W₁, M') (from h_eq and ExistsTask(W₁, V))
    rw [h_eq] at h_R_W₁V
    -- ExistsTask(W₁, M') = h_R_W₁V. ¬ExistsTask(M', W₁) = h_not_M'W₁.
