...
parse_args() {
  ...
  if $RUN_SOURCING || $RUN_SUPPRESSION || $RUN_CONDITIONALS || $RUN_ERROR_LOGGING || $RUN_UNBOUND_VARS || $RUN_HARD_BARRIER || $RUN_ARGUMENT_CAPTURE || $RUN_CHECKPOINTS || $RUN_README || $RUN_LINKS; then
    RUN_ALL=false
  fi
}
