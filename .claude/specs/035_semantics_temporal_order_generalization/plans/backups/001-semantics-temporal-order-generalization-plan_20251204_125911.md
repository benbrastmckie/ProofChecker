...
- **Research Reports**:
  ...
  - [Temporal Duality Options Comparison](../reports/temporal_duality_options_comparison.md) - **NEW**: Recommends Option A for is_valid monomorphization
...
## Phase 1: TaskFrame Generalization [IN PROGRESS]
dependencies: [0]
...
- [ ] `lake build ProofChecker.Semantics.TaskFrame` succeeds
...
- [ ] `lake build ProofChecker.Semantics.WorldHistory` succeeds
...
### Temporal Duality is_valid Fix (Option A - NOT STARTED)

**Current Issue**: The `is_valid` definition at line 544-546 is:
```lean
...
This polymorphic quantification over `{U : Type*}` causes 30+ universe level mismatch errors like:
```
application type mismatch
  @h_valid U
argument U has type Type u_2 but is expected to have type Type u_3
```

- [ ] **Fix `is_valid` to monomorphic definition (Option A)**
  - Goal: Remove `{U : Type*}` quantification, inherit `T` from section variable
  - New definition:
    ```lean
    private def is_valid (φ : Formula) : Prop :=
      ∀ (F : TaskFrame T) (M : TaskModel F) (τ : WorldHistory F) (t : T) (ht : τ.domain t),
        truth_at M τ t ht φ
    ```
  - Strategy: Change definition to use section's `T` parameter
  - Complexity: Simple (definition change)
  - Estimated: 5 minutes
  - **Reference**: Research report Section 7.1, Step 1

- [ ] **Simplify `valid_at_triple` theorem**
  - Goal: Remove `{U : Type*} [LinearOrderedAddCommGroup U]` from signature
  - Before:
    ```lean
  ...
  - After:
    ...
    ```
  - Strategy: Remove redundant type parameters, simplify proof body
  - Complexity: Simple (signature simplification)
  - Estimated: 10 minutes
  - **Reference**: Research report Section 7.1, Step 2

- [ ] **Simplify `truth_swap_of_valid_at_triple` theorem**
  - Goal: Remove `{U : Type*} [LinearOrderedAddCommGroup U]` from signature
  - Strategy: Replace all `F : TaskFrame U` with `F : TaskFrame T`, remove `@h_valid U _` applications
  - Complexity: Medium (15+ locations to update)
  - Estimated: 15 minutes
  - **Reference**: Research report Section 7.1, Step 3

- [ ] **Fix axiom swap validity theorems**
  - Goal: Update `swap_axiom_*_valid` theorems to use monomorphic `is_valid`
  - Strategy: Remove explicit type applications, use section `T`
  - Files affected: All 10 axiom swap theorems (MT, M4, MB, T4, TA, TL, MF, TF, prop_k, prop_s)
  - Complexity: Medium (mechanical changes across 10 theorems)
  - Estimated: 30 minutes

- [ ] **Fix rule preservation theorems**
  - Goal: Update `mp_preserves_swap_valid`, `modal_k_preserves_swap_valid`, `temporal_k_preserves_swap_valid`, `weakening_preserves_swap_valid`
  - Strategy: Remove explicit type applications, use section `T`
  - Complexity: Simple (4 theorems, mechanical changes)
  - Estimated: 15 minutes

- [ ] **Fix `axiom_swap_valid` master theorem**
  - Goal: Ensure all axiom cases use correct type inference
  - Strategy: Verify each case compiles after `is_valid` change
  - Complexity: Simple (verification)
  - Estimated: 10 minutes

- [ ] **Fix `derivable_implies_swap_valid` main theorem**
  - Goal: Update induction cases to use monomorphic `is_valid`
  - Strategy: Remove universe threading in all cases
  - Complexity: Medium (complex induction structure)
  - Estimated: 20 minutes

- [ ] **Fix deprecated `add_sub_cancel'` usages**
  - Goal: Replace deprecated `add_sub_cancel'` with `add_sub_cancel_left`
  - Strategy: Find-replace across file (12 occurrences at lines 310, 339, 350, 362, 364, 429, 433, 485, 489, 514, 519)
  - Complexity: Simple (mechanical replacement)
  - Estimated: 10 minutes

- [ ] **Fix `time_shift_preserves_truth` past case proof issue**
  - Goal: Fix line 429 where rewrite fails to find pattern `?a + ?b - ?a`
  - Strategy: Use `sub_add_cancel` or alternative group lemma approach
...
- [ ] `is_valid` is monomorphic (inherits section `T`) - **NEW**
- [ ] All axiom swap validity theorems compile without universe errors - **NEW**
- [ ] All rule preservation theorems compile without universe errors - **NEW**
- [ ] `derivable_implies_swap_valid` compiles without universe errors - **NEW**
- [ ] `lake build ProofChecker.Semantics.Truth` succeeds with zero errors
- [ ] All existing tests pass with `Int` instance

---

## Phase 4: Validity and Model Generalization [IN PROGRESS]
dependencies: [3]
...
- [ ] `lake build ProofChecker.Semantics` succeeds
...
- Phase 2: WorldHistory Generalization (15 hours) [COMPLETE] [depends on Phase 1]
- Phase 3: Truth Evaluation + is_valid Fix (12.5 hours) [IN PROGRESS] [depends on Phase 2]
  - Core truth evaluation: Complete
  - **is_valid Option A fix**: NOT STARTED (2.5 hours additional)
- Phase 4: Validity/Model (4 hours) [NOT STARTED] [depends on Phase 3 completion]
...
**CRITICAL**: Phase 4 (Validity and Model Generalization) CANNOT proceed until Phase 3's is_valid Option A fix is complete. The polymorphic `is_valid` causes universe level errors that will cascade to Phase 4.
...
4. **is_valid Universe Level Errors** (NEW - identified via research)
   - **Severity**: HIGH - currently 30+ errors blocking Phase 3 completion
   - **Root Cause**: Polymorphic `is_valid` definition quantifies over `{U : Type*}` creating nested universe conflicts
   - **Mitigation**: Implement Option A (monomorphic `is_valid`) as documented in Phase 3
   - **Reference**: `../reports/temporal_duality_options_comparison.md` Section 1.3 (Root Cause)
   - **Fallback**: Option B (explicit universe threading) NOT recommended - fundamental type theory issues
