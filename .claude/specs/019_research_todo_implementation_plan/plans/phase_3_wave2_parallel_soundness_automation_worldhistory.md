...
### Estimated Hours
15-18 hours (revised from 12-15 to include new Tasks 3A.6 and 3A.7 from task_semantics_alignment_analysis.md)
...
**Research Validation** (from task_semantics_alignment_analysis.md):
- ProofChecker implementation shows **excellent semantic alignment** with JPL paper (app:TaskSemantics)
- No fundamental misalignments found in core task semantics
- Paper proves P1/P2 perpetuity principles but doesn't claim TL/MF/TF are universally valid
- Conditional documentation approach aligns with paper's actual theorems

**Rationale** (from frame_constraints_soundness_analysis.md):
...
7. **Paper alignment**: JPL paper doesn't claim TL/MF/TF universal validity (only P1/P2)
...
grep -B 5 "Frame Constraint Required" ProofChecker/Metalogic/Soundness.lean
# Expected: 3 results (TL, MF, TF)

# Verify sorry still present (MVP approach keeps them with documentation)
grep -n "sorry" ProofChecker/Metalogic/Soundness.lean | grep -E "252|295|324"
...
grep -B 3 "Conditional Requirement\|Semantic Requirement" ProofChecker/Metalogic/Soundness.lean
# Expected: 3 results

# Verify sorry still present (MVP approach)
grep -n "sorry" ProofChecker/Metalogic/Soundness.lean | grep -E "398|416|431"
# Expected: 3 results
```
...
**File**: `ProofChecker/Metalogic/Soundness.lean`, `ProofCheckerTest/Metalogic/SoundnessTest.lean`
...
2. **Verify Frame Property Definitions Added**:
   ```bash
   grep -n "def BackwardPersistence\|def ModalTemporalPersistence" ProofChecker/Metalogic/Soundness.lean
   # Expected: 2 results (property definitions added)
   ```

3. **Verify Conditional Validity Documentation Complete**:
   ```bash
   grep -c "Frame Constraint Required\|Conditional Requirement\|Semantic Requirement" ProofChecker/Metalogic/Soundness.lean
...
#### Task 3A.6: Add Paper Cross-Reference Citations
**File**: `ProofChecker/Semantics/TaskFrame.lean`, `ProofChecker/Semantics/WorldHistory.lean`, `ProofChecker/Semantics/Truth.lean`, `ProofChecker/Metalogic/Soundness.lean`

**Objective**: Link ProofChecker implementation to JPL paper specification throughout codebase for traceability and verification

**Research Finding** (from task_semantics_alignment_analysis.md):
ProofChecker implementation faithfully implements paper's task semantics but cross-references to authoritative specification could be more explicit. Adding paper citations enables:
- Verification that implementation matches formal specification
- Traceability from code to published semantic theory
- Documentation of MVP simplifications (Int vs abstract abelian group)

**Implementation**:

**1. Update TaskFrame.lean Module Docstring** (add after line 3):
```lean
/-!
...
**Paper Definition (def:frame, line 1835)**:
- Frame: F = ⟨W, T, ⇒⟩
- World States: Nonempty set W
- Temporal Order: Totally ordered abelian group T = ⟨T, +, ≤⟩
- Task Relation: Parameterized relation ⇒ satisfying nullity and compositionality
...
**Semantic Alignment**: Core task relation structure (nullity, compositionality)
perfectly matches paper specification. Only difference is concrete Int vs
abstract group parameterization.
...
**2. Update WorldHistory.lean Module Docstring**:
```lean
/-!
# WorldHistory - World History Functions for TM Semantics

This module defines world histories, functions from convex time sets to world
states that respect the task relation.

## Paper Specification Reference

**Paper Definition (def:world-history, line 1849)**:
- World history: τ : X → W where X ⊆ T is convex
- Convexity: If z₁, z₂ ∈ X with z₁ ≤ z ≤ z₂, then z ∈ X (no gaps)
- Respects task relation: τ(x) ⇒ᵧ τ(x + y) for all x, y ∈ T where x, x+y ∈ X

**Implementation Correspondence**:
- `domain : Set Int` ↔ paper's "X ⊆ T"
- `toFun : ∀ (t : Int), t ∈ domain → F.WorldState` ↔ paper's "τ : X → W"
- `convex : IsConvex domain` ↔ paper's convexity requirement
- `respects_task` ↔ paper's "τ(x) ⇒ᵧ τ(x + y)"
...
**3. Update Truth.lean Module Docstring**:
```lean
/-!
# Truth - Truth Evaluation for TM Semantics

This module defines truth evaluation for formulas at model-history-time triples.

## Paper Specification Reference

**Paper Definition (def:BL-semantics, line 1857)**:
Truth at (M, τ, x):
- (pᵢ): M,τ,x ⊨ pᵢ  iff  x ∈ dom(τ) and τ(x) ∈ |pᵢ|
- (⊥):  M,τ,x ⊭ ⊥
- (→): M,τ,x ⊨ φ → ψ  iff  M,τ,x ⊭ φ or M,τ,x ⊨ ψ
- (□): M,τ,x ⊨ □φ  iff  M,σ,x ⊨ φ for all σ ∈ H_F
- (Past): M,τ,x ⊨ Past φ  iff  M,τ,y ⊨ φ for all y < x
- (Future): M,τ,x ⊨ Future φ  iff  M,τ,y ⊨ φ for all y > x
...
**4. Update Soundness.lean Module Docstring** (add reference to perpetuity principles):
```lean
/-!
# Soundness - Soundness Theorem for TM Logic

This module proves soundness: derivability implies semantic validity.

## Paper Specification Reference

**Perpetuity Principles (app:valid, line 1984)**:
The paper proves P1 (□φ → △φ) and P2 (▽φ → ◇φ) are valid over all task
semantic models using time-shift automorphisms.

**ProofChecker Extensions**:
This implementation extends beyond the paper's explicit proofs by including
axioms TL, MF, TF. These axioms are proven sound for specific frame classes
or documented as conditionally valid (Option B approach).

**Proven Axioms Aligned with Paper**: MT, M4, MB, T4, TA match paper's S5 modal
and linear temporal logic components.
...
**Expected Outcomes**:
- TaskFrame.lean: Paper reference to def:frame (line 1835), Int simplification documented
- WorldHistory.lean: Paper reference to def:world-history (line 1849)
- Truth.lean: Paper reference to def:BL-semantics (line 1857)
- Soundness.lean: Paper reference to app:valid (line 1984) for P1/P2 validity
- All 4 files have explicit paper cross-references for verification
...
grep -n "MVP Simplification" ProofChecker/Semantics/TaskFrame.lean
# Expected: 1 result
```
...
#### Task 3A.7: Verify Temporal Operator Semantics Match Paper
**File**: `ProofChecker/Semantics/Truth.lean`

**Objective**: Verify ProofChecker's temporal operator semantics exactly match JPL paper specification

**Research Finding** (from task_semantics_alignment_analysis.md):
Paper defines Past and Future operators with specific quantification over times in the history's domain. Need to verify ProofChecker implementation matches this exactly.
...
**Critical Question**: Does quantification restrict to `y ∈ τ.domain` or all `y ∈ Int`?
...
**Verification Steps**:
1. Read Truth.lean to examine `truth_at` definitions for `Formula.past` and `Formula.future`
2. Check if quantification is over `s ∈ τ.domain` (correct) or unrestricted `s ∈ Int` (incorrect)
3. Compare with paper's specification (lines 1864-1866)
4. If match found: Document verification success in this task
5. If mismatch found: Document as implementation gap requiring correction

**Expected Outcomes**:
- Verification complete: Temporal operators match paper (likely outcome)
- OR Implementation gap identified: Temporal quantification needs domain restriction (low probability)
- Documentation added to Truth.lean confirming paper alignment

**Testing**:
```bash
# Read temporal operator implementations
grep -A 10 "Formula.past\|Formula.future" ProofChecker/Semantics/Truth.lean

# Verify domain restriction present
grep "τ.domain" ProofChecker/Semantics/Truth.lean | grep -E "past|future"
# Expected: 2 results if domain restriction is present
```
...
**Total Tasks**: 7 (increased from 5 to add paper cross-references and temporal verification)
**Estimated Time**: 15-18 hours (increased from 12-15 based on task_semantics_alignment_analysis.md)
...
**Files Modified**:
- `ProofChecker/Metalogic/Soundness.lean` (frame property definitions + conditional documentation + paper references)
- `ProofChecker/Semantics/TaskFrame.lean` (paper cross-references + Int simplification documentation)
- `ProofChecker/Semantics/WorldHistory.lean` (paper cross-references)
- `ProofChecker/Semantics/Truth.lean` (paper cross-references + temporal operator verification)
**Files Created**:
- Tests in `ProofCheckerTest/Metalogic/SoundnessTest.lean`

**Key Research Findings Applied** (from task_semantics_alignment_analysis.md):
- **Excellent semantic alignment**: ProofChecker faithfully implements JPL paper's task semantics
- **No fundamental misalignments**: Core task relation structure perfectly matches paper
- **Int simplification validated**: Documented as valid special case of paper's abelian group requirement
- **Paper alignment confirmed**: JPL paper proves P1/P2 but doesn't claim TL/MF/TF universal validity
- **Enhancement opportunities**: Add paper cross-references and verify temporal operators
...
- [ ] Paper cross-references added to TaskFrame.lean, WorldHistory.lean, Truth.lean, Soundness.lean (Task 3A.6)
- [ ] Int simplification documented in TaskFrame.lean with paper correspondence
- [ ] Temporal operator semantics verified to match paper specification (Task 3A.7)
- [ ] lake build succeeds
