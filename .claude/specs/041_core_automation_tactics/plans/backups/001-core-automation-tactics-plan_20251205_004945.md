...
- **Date**: 2025-12-05
- **Feature**: Complete Task 7 (Implement Core Automation) - Implement remaining 8 ProofSearch.lean helper functions, verify Batteries compatibility for Aesop integration, and expand test suite to 75+ tests
- **Scope**: Complete automation infrastructure for Logos TM logic theorem prover. Implement 8 native Lean 4 helper functions for proof search (bounded_search, search_with_heuristics, search_with_cache, matches_axiom, find_implications_to, heuristic_score, box_context, future_context), verify and integrate Aesop automation framework if compatible, and expand test coverage from 50 to 75+ tests with comprehensive inference rule and search function testing. Output: Fully functional proof automation in Logos/Core/Automation/ with zero axiom stubs.
- **Status**: [NOT STARTED]
- **Estimated Hours**: 30-45 hours
- **Complexity Score**: 45 (extend existing: 10, 8 functions × 3 = 24, 2 files × 2 = 4, 2 complex implementations × 5 = 10, offset -3 for partial completion)
- **Structure Level**: 0
- **Estimated Phases**: 5
...
### Phase 4: Aesop Integration and Batteries Verification [NOT STARTED]
implementer: lean
lean_file: /home/benjamin/Documents/Philosophy/Projects/ProofChecker/Logos/Core/Automation/Tactics.lean
dependencies: []

**Objective**: Verify Batteries compatibility blocker is resolved, implement Aesop-based tm_auto with TM logic rule sets, and create AesopRules.lean module for forward chaining automation. If Batteries incompatible, document findings and keep native tm_auto as fallback.
...
- [ ] Verify Batteries compatibility with Truth.lean
  ...
  - Strategy: Add `import Aesop` to Truth.lean line 1. Run `lake build Logos/Core/Semantics/Truth.lean`. Check for type errors in truth_at or validity definitions. Document findings in task completion notes.
  - Complexity: Simple (verification task)
  - Estimated: 1 hour
  - Success: Build passes with zero errors
  - Failure: Document specific type errors, skip Aesop integration, keep native tm_auto
...
- [ ] Create Logos/Core/Automation/AesopRules.lean (if Batteries compatible)
  ...
  - Strategy: `declare_aesop_rule_sets [TMLogic]`. Create forward chaining lemmas for all 10 axioms: `@[aesop safe forward [TMLogic]] theorem modal_t_forward ...`. Mark perpetuity principles P1-P6 as forward rules. Mark inference rules as safe apply rules: `@[aesop safe apply [TMLogic]] theorem apply_modus_ponens ...`.
  - Complexity: Medium (requires forward chaining lemma proofs)
  - Dependencies: Batteries verification success
  - Estimated: 5-8 hours
  - Implementation Notes:
    - Forward lemmas apply axiom and return consequence (e.g., modal_t_forward: Derivable Γ (□φ) → Derivable Γ φ)
    - Use existing perpetuity proofs from Theorems/Perpetuity.lean
    - Safe apply rules for modus_ponens, modal_k, temporal_k

- [ ] Add normalization rules (if Batteries compatible)
  - Goal: `@[aesop norm unfold [TMLogic]]` for derived operators (diamond, always, sometimes)
  - Strategy: Mark derived operator definitions for unfolding. Skip equational simp lemmas (box_box_eq_box, future_future_eq_future) if soundness incomplete for MF/TF axioms. Alternative: unfold-only approach for MVP.
  ...
  - Implementation Notes:
    ...
    - Skip simp lemmas requiring MF/TF soundness proofs

- [ ] Replace tm_auto with Aesop version (if Batteries compatible)
  - Goal: Change Tactics.lean tm_auto from native `first` combinator to `aesop (rule_sets [TMLogic])`
  - Strategy: Replace lines 127-139 in Tactics.lean with `macro "tm_auto" : tactic => `(tactic| aesop (rule_sets [TMLogic]))`
  - Complexity: Simple (macro replacement)
  - Dependencies: AesopRules.lean complete
  - Estimated: 0.5 hours
  - Fallback: If Batteries incompatible, keep native tm_auto and document blocker

**Testing**:
```bash
# Batteries verification
lake build Logos/Core/Semantics/Truth.lean
# Expect: Zero errors if compatible, specific type errors if incompatible

# If compatible, build AesopRules
...
**Success Criteria**:
- [ ] Batteries compatibility verified (documented either way)
- [ ] If compatible: AesopRules.lean created with TMLogic rule set
- [ ] If compatible: All 10 axioms marked as forward rules
- [ ] If compatible: Perpetuity principles marked as forward rules
- [ ] If compatible: Inference rules marked as safe apply rules
- [ ] If compatible: tm_auto macro uses Aesop
- [ ] If incompatible: Native tm_auto retained, blocker documented
- [ ] Zero #lint warnings

**Expected Duration**: 9-13 hours (if Batteries compatible), 1-2 hours (if incompatible, verification only)
...
**Objective**: Expand TacticsTest.lean from 50 to 75+ tests with comprehensive coverage of inference rules, ProofSearch functions, propositional reasoning depth, and Aesop integration (if available).
...
- [ ] Inference Rule Tests (Tests 51-58, 8 tests)
  ...
  - Test 53: temporal_duality rule (if soundness available)
...
- [ ] Aesop Integration Tests (Tests 73-75, 3 tests) (if Phase 4 Aesop integration succeeded)
  ...
  - Complexity: Simple (if Aesop available)
  - Estimated: 1-2 hours
...
- [ ] Aesop integration tests pass (if Phase 4 succeeded)
...
1. **TODO.md** (lines 55-84):
   ...
   - Document all 8 ProofSearch implementations
   - Update effort estimates with actuals
   - Note Aesop integration status (success or documented blocker)
...
3. **KNOWN_LIMITATIONS.md**:
   ...
   - Document any remaining performance limitations (search depth bounds, cache size)
   - If Aesop blocked, document Batteries incompatibility with workaround (native tm_auto)

4. **TACTIC_DEVELOPMENT.md**:
   - Update tactic status table with completed implementations
   - Document ProofSearch function usage patterns
   - Add examples using bounded_search, search_with_heuristics
...
- Mitigation: Phase 4 verifies compatibility early. If blocked, native tm_auto remains functional.
- Contingency: Document specific errors, keep native implementation, defer Aesop to future work.
...
- Mitigation: Start with conservative depth bounds (3-5), use heuristic search for complex proofs, implement caching for repeated subgoals.
- Contingency: Document performance limits, recommend manual tactics for deep proofs.
...
- [ ] Phase 4: Batteries verified, Aesop integrated (or blocker documented)
