...
#rule-block("Formation")[
  Stipulates when $Pi_(x:A) B(x)$ is a valid type: we need $A$ to be a type and $B(x)$ to be a type for each $x : A$.
]
```
...
#rule-list(
  ("Formation",
   ...
   [Stipulates when $Pi_(x:A) B(x)$ is a valid type.]),
  ("Introduction",
   $frac(Gamma, x : A tack.r b(x) : B(x), Gamma tack.r lambda x. b(x) : Pi_(x:A) B(x))$,
   [Stipulates how to build a Pi-term via lambda abstraction.]),
)
```
...
- Works well with the thmbox theorem environments
...
This creates visual inconsistency where the first rule appears offset from subsequent rules.
