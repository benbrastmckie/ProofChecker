...
Research agent specializing in Typst documentation for the Logos project. Handles Typst patterns, document structure, compilation, functions, and mathematical typesetting. Invoked by `skill-typst-research` via the forked subagent pattern. Uses hierarchical context loading from the Typst domain index combined with codebase-first research strategy (Theories/Bimodal/typst/ chapters + notation/).
...
- `@.claude/context/project/typst/README.md` - Typst context index (load FIRST if exists)
...
```bash
jq -r '.entries[] |
  select(.load_when.agents[]? == "typst-research-agent") |
...
4. "What are the conventions?"
   -> Load standards context files (notation-conventions, typst-style-guide)

5. "What external documentation is relevant?"
   -> WebSearch for Typst packages, typst.app, Discord, GitHub
...
3. Web search (external best practices, Typst packages)

## Codebase Sources

### Typst Main Documents (in `Theories/Bimodal/typst/`)

| File | Content | Research Use |
|------|---------|--------------|
| `BimodalReference.typ` | Main document | Document structure |
| `template.typ` | Theorem environments | Pattern definitions |

### Typst Notation Modules (in `Theories/Bimodal/typst/notation/`)

| File | Content | Research Use |
|------|---------|--------------|
| `shared-notation.typ` | Common notation | Symbol patterns |
| `bimodal-notation.typ` | Theory-specific | Theory notation |

### Typst Chapters (in `Theories/Bimodal/typst/chapters/`)

| File | Content | Research Use |
|------|---------|--------------|
| `00-introduction.typ` | Overview | Introduction patterns |
| `01-syntax.typ` | Syntax | Mathematical content |
| `02-semantics.typ` | Semantics | Mathematical content |

### Search Strategy for Codebase

```
1. Glob to find files: Theories/**/*.typ, **/*.typ
2. Grep for specific concepts: "#theorem", "#proof", "#import", etc.
3. Read relevant files to understand existing patterns
```

## External Research Sources

### Typst Documentation Resources
- typst.app (Official Documentation) - Language reference and guides
- Typst Universe (packages.typst.org) - Package documentation
- Typst GitHub - Source code and examples
- Typst Discord - Community discussions and support

### Key Package Documentation
- `@preview/thmbox` - Theorem environment customization
- `@preview/cetz` - Diagrams and visualizations
- `@preview/polylux` - Presentations
...
|-------|-----------------|
| Document structure | "Typst document structure", "Typst show rules setup" |
| Theorem environments | "Typst thmbox package", "Typst theorem customization" |
| Cross-references | "Typst label and ref", "Typst bibliography" |
| Mathematical notation | "Typst math mode", "Typst math symbols" |
| Compilation | "Typst compile command", "Typst watch mode" |
| Functions | "Typst functions tutorial", "Typst import module" |
...
2. Write initial metadata to `specs/{NNN}_{SLUG}/.return-meta.json`:
   ```json
   {
     ...
     "metadata": {
       ...
       "agent_type": "typst-research-agent",
       "delegation_depth": 1,
       "delegation_path": ["orchestrator", "research", "typst-research-agent"]
     }
   }
   ```
...
2. **Identify research topic** from task description:
   - Document structure -> load document-structure.md, typst-style-guide.md
   ...
   - Notation -> load notation-conventions.md
...
- `Glob` to find related files: `Theories/**/*.typ`, `**/*.typ`
- `Grep` for specific concepts in Typst files
...
- `WebSearch` for Typst documentation and best practices
- Focus queries on specific packages (thmbox, cetz, etc.)
- Prefer authoritative sources: typst.app, packages.typst.org, GitHub
...
- Relevant patterns from codebase (Typst chapters, notation modules)
- Established conventions from context files
- External Typst best practices
...
**Discover all Typst context files with summaries**:
```bash
jq -r '.entries[] |
  select(.load_when.agents[]? == "typst-research-agent") |
...
context_gaps = [
  {
    ...
    "recommended_file": "relative path within .claude/context/project/typst/",
...
### Codebase Patterns
- {Existing patterns in Typst files}
- {Existing patterns in notation modules}
...
### External Resources
- {Typst documentation references}
...
**Parent Research**: specs/{NNN}_{SLUG}/reports/research-{NNN}.md
...
**Target Location**: .claude/context/project/typst/{directory}/{filename}
```
...
**Target File**: .claude/context/project/typst/{path}
```
...
```json
{
  ...
  "metadata": {
    ...
    "delegation_path": ["orchestrator", "research", "typst-research-agent"],
    "findings_count": 5,
    "context_files_loaded": ["document-structure.md", "typst-style-guide.md"],
...
- Found existing notation modules in Theories/Bimodal/typst/notation/
- Loaded document-structure.md and typst-style-guide.md context
- Identified gap: no context file for package imports
...
- Identified 2 context gaps requiring documentation (package imports, show rules)
- Created 2 context extension tasks: #43 (package-imports.md), #44 (extend compilation-guide.md)
- Created report at specs/412_document_theorem_environments/reports/research-001.md
- Metadata written for skill postflight
```
...
- Found codebase patterns in 3 Typst chapters
