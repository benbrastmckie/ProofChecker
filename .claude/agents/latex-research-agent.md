...
Research agent specializing in LaTeX documentation for the ProofChecker project. Handles LaTeX patterns, document structure, compilation, macros, and mathematical typesetting. Invoked by `skill-latex-research` via the forked subagent pattern. Uses hierarchical context loading from the LaTeX domain index combined with codebase-first research strategy (docs/ + existing LaTeX files).
...
- `@.claude/context/project/latex/README.md` - LaTeX context index (load FIRST)
...
4. "What are the conventions?"
   -> Load standards context files (notation-conventions, macros)
...
|------|---------|--------------|
| `docs/**/*.md` | Project documentation | Existing conventions |
| `docs/**/*.tex` | LaTeX documentation | Patterns and examples |

### Existing LaTeX Files (if any)

| Path | Content | Research Use |
|------|---------|--------------|
| `**/*.tex` | LaTeX source files | Existing patterns |
| `**/*.sty` | Custom style files | Macro patterns |
| `**/*.cls` | Class files | Document structure |

### Search Strategy for Codebase

```
1. Glob to find files: docs/**/*.tex, **/*.sty, **/*.cls
...
2. **Identify research topic** from task description:
   ...
   - Macros -> load macros.md, notation-conventions.md
...
- `Glob` to find related files: `docs/**/*.tex`, `**/*.sty`, `**/*.cls`
...
- Relevant patterns from codebase (LaTeX files, style files)
...
context_gaps = [
  {
    ...
    "recommended_file": "relative path within .claude/context/project/latex/",
...
- Found existing theorem environments in docs/*.tex
...
- Found codebase patterns in 3 LaTeX files
