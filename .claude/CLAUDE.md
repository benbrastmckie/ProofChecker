...
**Quick Diagnosis**:
- **Symptom**: No progress for >3 minutes, frozen at MCP tool call
- **Cause**: Zombie lake processes or concurrent builds ([Issue #115](https://github.com/oOo0oOo/lean-lsp-mcp/issues/115), [Issue #118](https://github.com/oOo0oOo/lean-lsp-mcp/issues/118))
- **Fix**: Run `/refresh`, then retry `/implement`

**Detection**:
```bash
# Check for stale progress
find specs -name ".agent-progress.json" -mmin +3

# Check for concurrent builds (should be ≤1)
pgrep -fc "lake build"

# Check for zombies (should be 0)
ps aux | grep -c '[l]ake.*Z'
```

**Log Files**:
- `.claude/logs/subagent-postflight.log` - Deadlock detection events
...
- **Concurrent builds**: Check postflight log for "concurrent lake build" warnings
- **Zombie accumulation**: Check cleanup log for trends (target: <2 per session)
- **Diagnostic throttling**: Check progress file for `diagnostic_calls` vs `skipped_diagnostics`

**Upstream Links**:
- [Issue #115](https://github.com/oOo0oOo/lean-lsp-mcp/issues/115) - lean_diagnostic_messages hangs (fixed in Lean v4.26.0+)
- [Issue #118](https://github.com/oOo0oOo/lean-lsp-mcp/issues/118) - Concurrent build memory exhaustion

**Complete Guide**: See `.claude/docs/diagnostics/lean-hang-mitigation.md` for comprehensive troubleshooting.
