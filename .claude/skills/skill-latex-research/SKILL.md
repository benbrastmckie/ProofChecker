...
description: Research LaTeX documentation tasks using domain context and codebase exploration. Invoke for latex-language research involving document structure, theorem environments, cross-references, compilation, and LaTeX best practices.
...
Thin wrapper that delegates LaTeX documentation research to `latex-research-agent` subagent.
...
- Path: `.claude/context/core/templates/context-knowledge-template.md` - Context knowledge extraction template
...
- Research involves document structure, theorem environments, cross-references, compilation, or LaTeX best practices
- Domain context files are needed for LaTeX documentation
...
```json
{
  ...
  "delegation_depth": 1,
  "delegation_path": ["orchestrator", "research", "skill-latex-research"],
  "timeout": 3600,
  "task_context": {
    "task_number": N,
...
Tool: Task (NOT Skill)
Parameters:
  - subagent_type: "latex-research-agent"
  - prompt: [Include task_context, delegation_context, focus_prompt, metadata_file_path]
  - description: "Execute LaTeX research for task {N}"
```

**DO NOT** use `Skill(latex-research-agent)` - this will FAIL.

The subagent will:
- Load domain context files from `.claude/context/project/latex/`
...
- Found existing document patterns in docs/*.tex
...
- Found patterns in 3 LaTeX files before timeout
