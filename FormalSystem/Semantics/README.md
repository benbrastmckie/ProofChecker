...
| StarDeterminism.lean | `app:deterministic-future`'s positive half (`sentDet_of_deterministic`) and Theorem C's `Det-pm` half: `star_congr_of_deterministic`, `detPM`, `detPM_unfold`, `detPM_of_deterministic`, `deterministic_of_detPM`, `deterministic_starDefinable` — the last two theorems of ZFC |
