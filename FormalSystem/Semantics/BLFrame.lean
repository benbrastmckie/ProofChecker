...
/-!
# `BLFrame` — a native BL frame notion, not bound to `TaskFrame`
...
order-reversal transfer lemma. Everything here is additive: `BLTruthAt`, `BLValid` and the whole
`TaskFrame`-bound semantic stack are untouched and sit beside this layer.

## Why a native frame notion is required

`Semantics/BLTruth.lean`'s `BLTruthAt` evaluates at a `TaskModel F` for `F : TaskFrame`, and a
...
`blValid_sp` go through: on any ordered abelian group the order is either densely ordered or has
...
A `BLFrame` keeps only what TM's schemata actually need of time: a nonempty point set with a
transitive irreflexive relation that is unbounded in both directions and forward- and
backward-linear. No group, no successor, no completeness — so a `BLFrame` may mix order shapes,
which is what a countermodel to `(Sp)` requires.

## `□` is the universal modality

`BLFrameTruth`'s `box` clause quantifies over *all* points of the frame:

  `BLFrameTruth F V w (□φ) ↔ ∀ v : F.Point, BLFrameTruth F V v φ`
...
under order reversal (`BLFrame.swap`). That closure is what makes `truth_swap` available, and
...
- `BLFrame`: the native frame notion — a nonempty point set with an unbounded, transitive,
  irreflexive, forward- and backward-linear strict order
- `BLFrame.swap`: order reversal, witnessing converse closure of the class
- `BLFrameTruth`: truth of a `BLFormula` at a point of a `BLFrame` under a valuation, by
  six-clause recursion with `□` read as the universal modality
- `BLFrameValid`: validity — truth at every point of every `BLFrame` under every valuation

## Main Results

`BLFrameTruth.*` — characterization lemmas mirroring `Semantics/BLTruth.lean`'s `BLTruth`
...
* `FormalSystem/Semantics/BLTruth.lean` — the `TaskFrame`-bound recursion this sits beside
...
frame · base-language · BLFrame · universal-modality · order-reversal
-/
...
(`BLFrame.swap`); that closure discharges the temporal-duality rule.
-/
...
Used only by `BLFrame.swap`: reversing the order turns `fut_lin`'s conclusion into `past_lin`'s
...
/--
Order reversal on a `BLFrame`: keep the points, invert `lt`.

The class of `BLFrame`s is closed under this operation — `no_max` and `no_min` swap roles, as do
...
/--
Truth of a base-language formula at a point of a `BLFrame` under a valuation.

Six clauses, one per `BLFormula` constructor. The temporal clauses are strict, exactly as in
`BLTruthAt`. The `box` clause is the **universal modality** over `Point` — see the module
...
/--
Validity on the native BL frame class: truth at every point of every `BLFrame` under every
valuation.

This is the notion native BL soundness (`Metalogic/blFrameValid_of_derivation`) concludes, and the
...
theorem bot_false : ¬ BLFrameTruth F V w BLFormula.bot := id
...
past, present and future. The association mirrors `BLFormula.always`. -/
@[simp] theorem always_iff (φ : BLFormula) :
    BLFrameTruth F V w φ.always ↔
      (∀ v, F.lt v w → BLFrameTruth F V v φ) ∧ BLFrameTruth F V w φ ∧
        (∀ v, F.lt w v → BLFrameTruth F V v φ) := by
...
congruences, and the two temporal clauses trade places exactly as `BLFormula.swapBL` does. This
