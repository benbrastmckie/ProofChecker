...
theorem sameStateAt_iff_of_total {τ σ : WorldHistory F} (hτ : τ.IsTotal) (hσ : σ.IsTotal)
    (t : F.Duration) :
    SameStateAt τ σ t ↔ τ.states t (hτ t) = σ.states t (hσ t) :=
  ⟨fun h => h _ _, fun h _ _ => h⟩

theorem SameStateAt.refl (τ : WorldHistory F) (t : F.Duration) : SameStateAt τ τ t :=
  fun _ _ => rfl

theorem SameStateAt.symm {τ σ : WorldHistory F} {t : F.Duration} (h : SameStateAt τ σ t) :
    SameStateAt σ τ t :=
  fun hσ hτ => (h hτ hσ).symm

/-- Transitivity, given that the middle history is defined at `t` (automatic at a total one). -/
theorem SameStateAt.trans {τ σ ρ : WorldHistory F} {t : F.Duration} (hσ : σ.domain t)
...
theorem box_iff (φ : StarFormula) :
    StarTruthAt M τ t (.box φ) ↔ ∀ σ : WorldHistory F, σ.IsTotal → StarTruthAt M σ t φ := Iff.rfl
...
theorem stab_iff (φ : StarFormula) :
    StarTruthAt M τ t (.stab φ) ↔
      ∀ σ : WorldHistory F, σ.IsTotal → SameStateAt τ σ t → StarTruthAt M σ t φ := Iff.rfl
...
theorem diamond_iff (φ : StarFormula) :
    StarTruthAt M τ t (diamond φ) ↔ ∃ σ : WorldHistory F, σ.IsTotal ∧ StarTruthAt M σ t φ := by
